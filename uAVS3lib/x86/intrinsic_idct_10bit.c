...
void add_inv_trans_4x4_sec_10bit_sse128(coef_t *src, pel_t *pred, int i_pred, pel_t *dst, int i_dst, int bit_depth)
{
	...
	__m128i m128iS0, m128iS1, P0, P1, P2, P3;
	__m128i zero = _mm_setzero_si128();
	__m128i factor, min_val, max_val;
	int shift2 = 22 - bit_depth;
	int rnd_factor2 = shift2 == 0 ? 0 : 1 << (shift2 - 1);
	short min_pixel = -(1 << (bit_depth+1));
	short max_pixel = (1 << (bit_depth + 1)) - 1;
	short max_pixel2 = (1 << bit_depth) - 1;
	...
	factor = _mm_set1_epi32(rnd_factor2);
	...
	tmpSum3 = _mm_add_epi32(tmpSum2, factor);
	//shift operation
	tmpDst0 = _mm_packs_epi32(_mm_srai_epi32(tmpSum3, shift2), zero);			//!only low 64bits (4xSHORT) are valid!
	...
	tmpSum3 = _mm_add_epi32(tmpSum2, factor);
	//shift operation
	tmpDst1 = _mm_packs_epi32(_mm_srai_epi32(tmpSum3, shift2), zero);			//!only low 64bits (4xSHORT) are valid!
	...
	tmpSum3 = _mm_add_epi32(tmpSum2, factor);
	//shift operation
	tmpDst2 = _mm_packs_epi32(_mm_srai_epi32(tmpSum3, shift2), zero);			//!only low 64bits (4xSHORT) are valid!
	...
	tmpSum3 = _mm_add_epi32(tmpSum2, factor);
	//shift operation
	tmpDst3 = _mm_packs_epi32(_mm_srai_epi32(tmpSum3, shift2), zero);				//!only low 64bits (4xSHORT) are valid!
	...
	//clip
	max_val = _mm_set1_epi16(max_pixel);
	min_val = _mm_set1_epi16(min_pixel);
	...
	//pix_add
	P0 = _mm_loadl_epi64((__m128i*)&pred[0]);
	P1 = _mm_loadl_epi64((__m128i*)&pred[i_pred]);
	P2 = _mm_loadl_epi64((__m128i*)&pred[i_pred << 1]);
	P3 = _mm_loadl_epi64((__m128i*)&pred[i_pred * 3]);
	...
	max_val = _mm_set1_epi16(max_pixel2);
	m128iS0 = _mm_max_epi16(m128iS0, zero);
	m128iS0 = _mm_min_epi16(m128iS0, max_val);
	m128iS1 = _mm_max_epi16(m128iS1, zero);
	m128iS1 = _mm_min_epi16(m128iS1, max_val);

	_mm_storel_epi64((__m128i*)&dst[0], m128iS0);
	_mm_storeh_pi((__m64*)&dst[i_dst], _mm_castsi128_ps(m128iS0));
	_mm_storel_epi64((__m128i*)&dst[i_dst << 1], m128iS1);
	_mm_storeh_pi((__m64*)&dst[i_dst * 3], _mm_castsi128_ps(m128iS1));
}
...
void add_inv_trans_8x8_10bit_sse128(coef_t *src, pel_t *pred, int i_pred, pel_t *dst, int i_dst, int bit_depth)
{
	...
	m128iAdd = _mm_set1_epi32(add2);						//设置四舍五入
	...
	//clip
	{
		max_val = _mm_set1_epi16(vmax);
		min_val = _mm_set1_epi16(vmin);
...

	{
		...
		P0 = _mm_loadu_si128((__m128i*)&pred[0]);
		P1 = _mm_loadu_si128((__m128i*)&pred[i_pred]);

		D = _mm_max_epi16(_mm_add_epi16(T10, P0), zero);
		D = _mm_min_epi16(D, max_val);
		_mm_storeu_si128((__m128i*)&dst[0], D);                   // [70 60 50 40 30 20 10 00]

		D = _mm_max_epi16(_mm_add_epi16(T11, P1), zero);
		D = _mm_min_epi16(D, max_val);
		_mm_storeu_si128((__m128i*)&dst[i_dst], D);				   // [71 61 51 41 31 21 11 01]
		...
		P0 = _mm_loadu_si128((__m128i*)&pred[i_pred * 2]);
		P1 = _mm_loadu_si128((__m128i*)&pred[i_pred * 3]);		
		D = _mm_max_epi16(_mm_add_epi16(T10, P0), zero);
		D = _mm_min_epi16(D, max_val);
		_mm_storeu_si128((__m128i*)&dst[i_dst * 2], D);
		D = _mm_max_epi16(_mm_add_epi16(T11, P1), zero);
		D = _mm_min_epi16(D, max_val);
		_mm_storeu_si128((__m128i*)&dst[i_dst * 3], D);
		...
		P0 = _mm_loadu_si128((__m128i*)&pred[i_pred * 4]);
		P1 = _mm_loadu_si128((__m128i*)&pred[i_pred * 5]);
		D = _mm_max_epi16(_mm_add_epi16(T10, P0), zero);
		D = _mm_min_epi16(D, max_val);
		_mm_storeu_si128((__m128i*)&dst[i_dst * 4], D);
		D = _mm_max_epi16(_mm_add_epi16(T11, P1), zero);
		D = _mm_min_epi16(D, max_val);
		_mm_storeu_si128((__m128i*)&dst[i_dst * 5], D);
		...
		P0 = _mm_loadu_si128((__m128i*)&pred[i_pred * 6]);
		P1 = _mm_loadu_si128((__m128i*)&pred[i_pred * 7]);
		D = _mm_max_epi16(_mm_add_epi16(T10, P0), zero);
		D = _mm_min_epi16(D, max_val);
		_mm_storeu_si128((__m128i*)&dst[i_dst * 6], D);
		D = _mm_max_epi16(_mm_add_epi16(T11, P1), zero);
		D = _mm_min_epi16(D, max_val);
		_mm_storeu_si128((__m128i*)&dst[i_dst * 7], D);
...
void add_inv_trans_16x16_10bit_sse128(coef_t *src, pel_t *pred, int i_pred, pel_t *dst, int i_dst, int bit_depth)
{
	...
	for (pass = 0; pass < 2; pass++)
	{
		if (pass == 1)
		{
			c32_rnd = _mm_set1_epi32(add2);				// pass == 1 第二次四舍五入
			nShift = shift2;
		}
...
#undef TRANSPOSE_8x8_16BIT
	...
	//clip
	{
		__m128i max_val = _mm_set1_epi16(vmax);
		__m128i min_val = _mm_set1_epi16(vmin);
	...
	res00[0] = _mm_loadu_si128((const __m128i*)&pred[0]);
	res01[0] = _mm_loadu_si128((const __m128i*)&pred[1 * i_pred]);
	res02[0] = _mm_loadu_si128((const __m128i*)&pred[2 * i_pred]);
	res03[0] = _mm_loadu_si128((const __m128i*)&pred[3 * i_pred]);
	res04[0] = _mm_loadu_si128((const __m128i*)&pred[4 * i_pred]);
	res05[0] = _mm_loadu_si128((const __m128i*)&pred[5 * i_pred]);
	res06[0] = _mm_loadu_si128((const __m128i*)&pred[6 * i_pred]);
	res07[0] = _mm_loadu_si128((const __m128i*)&pred[7 * i_pred]);
	res08[0] = _mm_loadu_si128((const __m128i*)&pred[8 * i_pred]);
	res09[0] = _mm_loadu_si128((const __m128i*)&pred[9 * i_pred]);
	res10[0] = _mm_loadu_si128((const __m128i*)&pred[10 * i_pred]);
	res11[0] = _mm_loadu_si128((const __m128i*)&pred[11 * i_pred]);
	res12[0] = _mm_loadu_si128((const __m128i*)&pred[12 * i_pred]);
	res13[0] = _mm_loadu_si128((const __m128i*)&pred[13 * i_pred]);
	res14[0] = _mm_loadu_si128((const __m128i*)&pred[14 * i_pred]);
	res15[0] = _mm_loadu_si128((const __m128i*)&pred[15 * i_pred]);

	res00[1] = _mm_loadu_si128((const __m128i*)&pred[0 + 8]);
	res01[1] = _mm_loadu_si128((const __m128i*)&pred[1 * i_pred + 8]);
	res02[1] = _mm_loadu_si128((const __m128i*)&pred[2 * i_pred + 8]);
	res03[1] = _mm_loadu_si128((const __m128i*)&pred[3 * i_pred + 8]);
	res04[1] = _mm_loadu_si128((const __m128i*)&pred[4 * i_pred + 8]);
	res05[1] = _mm_loadu_si128((const __m128i*)&pred[5 * i_pred + 8]);
	res06[1] = _mm_loadu_si128((const __m128i*)&pred[6 * i_pred + 8]);
	res07[1] = _mm_loadu_si128((const __m128i*)&pred[7 * i_pred + 8]);
	res08[1] = _mm_loadu_si128((const __m128i*)&pred[8 * i_pred + 8]);
	res09[1] = _mm_loadu_si128((const __m128i*)&pred[9 * i_pred + 8]);
	res10[1] = _mm_loadu_si128((const __m128i*)&pred[10 * i_pred + 8]);
	res11[1] = _mm_loadu_si128((const __m128i*)&pred[11 * i_pred + 8]);
	res12[1] = _mm_loadu_si128((const __m128i*)&pred[12 * i_pred + 8]);
	res13[1] = _mm_loadu_si128((const __m128i*)&pred[13 * i_pred + 8]);
	res14[1] = _mm_loadu_si128((const __m128i*)&pred[14 * i_pred + 8]);
	res15[1] = _mm_loadu_si128((const __m128i*)&pred[15 * i_pred + 8]);
	...
	{
		__m128i max_val = _mm_set1_epi16(vmax);
		__m128i min_val = zero;
		res00[0] = _mm_min_epi16(res00[0], max_val);
		res00[0] = _mm_max_epi16(res00[0], min_val);
		res00[1] = _mm_min_epi16(res00[1], max_val);
		res00[1] = _mm_max_epi16(res00[1], min_val);

		res01[0] = _mm_min_epi16(res01[0], max_val);
		res01[0] = _mm_max_epi16(res01[0], min_val);
		res01[1] = _mm_min_epi16(res01[1], max_val);
		res01[1] = _mm_max_epi16(res01[1], min_val);

		res02[0] = _mm_min_epi16(res02[0], max_val);
		res02[0] = _mm_max_epi16(res02[0], min_val);
		res02[1] = _mm_min_epi16(res02[1], max_val);
		res02[1] = _mm_max_epi16(res02[1], min_val);

		res03[0] = _mm_min_epi16(res03[0], max_val);
		res03[0] = _mm_max_epi16(res03[0], min_val);
		res03[1] = _mm_min_epi16(res03[1], max_val);
		res03[1] = _mm_max_epi16(res03[1], min_val);

		res04[0] = _mm_min_epi16(res04[0], max_val);
		res04[0] = _mm_max_epi16(res04[0], min_val);
		res04[1] = _mm_min_epi16(res04[1], max_val);
		res04[1] = _mm_max_epi16(res04[1], min_val);

		res05[0] = _mm_min_epi16(res05[0], max_val);
		res05[0] = _mm_max_epi16(res05[0], min_val);
		res05[1] = _mm_min_epi16(res05[1], max_val);
		res05[1] = _mm_max_epi16(res05[1], min_val);

		res06[0] = _mm_min_epi16(res06[0], max_val);
		res06[0] = _mm_max_epi16(res06[0], min_val);
		res06[1] = _mm_min_epi16(res06[1], max_val);
		res06[1] = _mm_max_epi16(res06[1], min_val);

		res07[0] = _mm_min_epi16(res07[0], max_val);
		res07[0] = _mm_max_epi16(res07[0], min_val);
		res07[1] = _mm_min_epi16(res07[1], max_val);
		res07[1] = _mm_max_epi16(res07[1], min_val);

		res08[0] = _mm_min_epi16(res08[0], max_val);
		res08[0] = _mm_max_epi16(res08[0], min_val);
		res08[1] = _mm_min_epi16(res08[1], max_val);
		res08[1] = _mm_max_epi16(res08[1], min_val);

		res09[0] = _mm_min_epi16(res09[0], max_val);
		res09[0] = _mm_max_epi16(res09[0], min_val);
		res09[1] = _mm_min_epi16(res09[1], max_val);
		res09[1] = _mm_max_epi16(res09[1], min_val);

		res10[0] = _mm_min_epi16(res10[0], max_val);
		res10[0] = _mm_max_epi16(res10[0], min_val);
		res10[1] = _mm_min_epi16(res10[1], max_val);
		res10[1] = _mm_max_epi16(res10[1], min_val);

		res11[0] = _mm_min_epi16(res11[0], max_val);
		res11[0] = _mm_max_epi16(res11[0], min_val);
		res11[1] = _mm_min_epi16(res11[1], max_val);
		res11[1] = _mm_max_epi16(res11[1], min_val);

		res12[0] = _mm_min_epi16(res12[0], max_val);
		res12[0] = _mm_max_epi16(res12[0], min_val);
		res12[1] = _mm_min_epi16(res12[1], max_val);
		res12[1] = _mm_max_epi16(res12[1], min_val);

		res13[0] = _mm_min_epi16(res13[0], max_val);
		res13[0] = _mm_max_epi16(res13[0], min_val);
		res13[1] = _mm_min_epi16(res13[1], max_val);
		res13[1] = _mm_max_epi16(res13[1], min_val);

		res14[0] = _mm_min_epi16(res14[0], max_val);
		res14[0] = _mm_max_epi16(res14[0], min_val);
		res14[1] = _mm_min_epi16(res14[1], max_val);
		res14[1] = _mm_max_epi16(res14[1], min_val);

		res15[0] = _mm_min_epi16(res15[0], max_val);
		res15[0] = _mm_max_epi16(res15[0], min_val);
		res15[1] = _mm_min_epi16(res15[1], max_val);
		res15[1] = _mm_max_epi16(res15[1], min_val);
	}

	_mm_storeu_si128((__m128i*)&dst[0], res00[0]);
	...
	_mm_storeu_si128((__m128i*)&dst[4 * i_dst], res04[0]);
	_mm_storeu_si128((__m128i*)&dst[5 * i_dst], res05[0]);
	_mm_storeu_si128((__m128i*)&dst[6 * i_dst], res06[0]);
	_mm_storeu_si128((__m128i*)&dst[7 * i_dst], res07[0]);
	_mm_storeu_si128((__m128i*)&dst[8 * i_dst], res08[0]);
	_mm_storeu_si128((__m128i*)&dst[9 * i_dst], res09[0]);
	_mm_storeu_si128((__m128i*)&dst[10 * i_dst], res10[0]);
	_mm_storeu_si128((__m128i*)&dst[11 * i_dst], res11[0]);
	_mm_storeu_si128((__m128i*)&dst[12 * i_dst], res12[0]);
	_mm_storeu_si128((__m128i*)&dst[13 * i_dst], res13[0]);
	_mm_storeu_si128((__m128i*)&dst[14 * i_dst], res14[0]);
	_mm_storeu_si128((__m128i*)&dst[15 * i_dst], res15[0]);	
	_mm_storeu_si128((__m128i*)&dst[8], res00[1]);
	...
	_mm_storeu_si128((__m128i*)&dst[4 * i_dst + 8], res04[1]);
	_mm_storeu_si128((__m128i*)&dst[5 * i_dst + 8], res05[1]);
	_mm_storeu_si128((__m128i*)&dst[6 * i_dst + 8], res06[1]);
	_mm_storeu_si128((__m128i*)&dst[7 * i_dst + 8], res07[1]);
	_mm_storeu_si128((__m128i*)&dst[8 * i_dst + 8], res08[1]);
	_mm_storeu_si128((__m128i*)&dst[9 * i_dst + 8], res09[1]);
	_mm_storeu_si128((__m128i*)&dst[10 * i_dst + 8], res10[1]);
	_mm_storeu_si128((__m128i*)&dst[11 * i_dst + 8], res11[1]);
	_mm_storeu_si128((__m128i*)&dst[12 * i_dst + 8], res12[1]);
	_mm_storeu_si128((__m128i*)&dst[13 * i_dst + 8], res13[1]);
	_mm_storeu_si128((__m128i*)&dst[14 * i_dst + 8], res14[1]);
	_mm_storeu_si128((__m128i*)&dst[15 * i_dst + 8], res15[1]);
}
...
void add_inv_trans_4x4_10bit_sse128(coef_t *src, pel_t *pred, int i_pred, pel_t *dst, int i_dst, int bit_depth)
{
	...
	c32_rnd = _mm_set1_epi32(add2);						//设置四舍五入
	...
	m128iS0 = _mm_packs_epi32(_mm_srai_epi32(_mm_add_epi32(E0, O0), shift2), _mm_srai_epi32(_mm_sub_epi32(E1, O1), shift2));
	m128iS1 = _mm_packs_epi32(_mm_srai_epi32(_mm_add_epi32(E1, O1), shift2), _mm_srai_epi32(_mm_sub_epi32(E0, O0), shift2));
...
#undef TRANSPOSE_8x8_16BIT
	...
	{
		int offset = (k << 3);
		res00[k] = _mm_loadu_si128((const __m128i*)&pred[0 + offset]);
		res01[k] = _mm_loadu_si128((const __m128i*)&pred[1 * i_pred + offset]);
		res02[k] = _mm_loadu_si128((const __m128i*)&pred[2 * i_pred + offset]);
		res03[k] = _mm_loadu_si128((const __m128i*)&pred[3 * i_pred + offset]);
		res04[k] = _mm_loadu_si128((const __m128i*)&pred[4 * i_pred + offset]);
		res05[k] = _mm_loadu_si128((const __m128i*)&pred[5 * i_pred + offset]);
		res06[k] = _mm_loadu_si128((const __m128i*)&pred[6 * i_pred + offset]);
		res07[k] = _mm_loadu_si128((const __m128i*)&pred[7 * i_pred + offset]);
		res08[k] = _mm_loadu_si128((const __m128i*)&pred[8 * i_pred + offset]);
		res09[k] = _mm_loadu_si128((const __m128i*)&pred[9 * i_pred + offset]);
		res10[k] = _mm_loadu_si128((const __m128i*)&pred[10 * i_pred + offset]);
		res11[k] = _mm_loadu_si128((const __m128i*)&pred[11 * i_pred + offset]);
		res12[k] = _mm_loadu_si128((const __m128i*)&pred[12 * i_pred + offset]);
		res13[k] = _mm_loadu_si128((const __m128i*)&pred[13 * i_pred + offset]);
		res14[k] = _mm_loadu_si128((const __m128i*)&pred[14 * i_pred + offset]);
		res15[k] = _mm_loadu_si128((const __m128i*)&pred[15 * i_pred + offset]);
		res16[k] = _mm_loadu_si128((const __m128i*)&pred[16 * i_pred + offset]);
		res17[k] = _mm_loadu_si128((const __m128i*)&pred[17 * i_pred + offset]);
		res18[k] = _mm_loadu_si128((const __m128i*)&pred[18 * i_pred + offset]);
		res19[k] = _mm_loadu_si128((const __m128i*)&pred[19 * i_pred + offset]);
		res20[k] = _mm_loadu_si128((const __m128i*)&pred[20 * i_pred + offset]);
		res21[k] = _mm_loadu_si128((const __m128i*)&pred[21 * i_pred + offset]);
		res22[k] = _mm_loadu_si128((const __m128i*)&pred[22 * i_pred + offset]);
		res23[k] = _mm_loadu_si128((const __m128i*)&pred[23 * i_pred + offset]);
		res24[k] = _mm_loadu_si128((const __m128i*)&pred[24 * i_pred + offset]);
		res25[k] = _mm_loadu_si128((const __m128i*)&pred[25 * i_pred + offset]);
		res26[k] = _mm_loadu_si128((const __m128i*)&pred[26 * i_pred + offset]);
		res27[k] = _mm_loadu_si128((const __m128i*)&pred[27 * i_pred + offset]);
		res28[k] = _mm_loadu_si128((const __m128i*)&pred[28 * i_pred + offset]);
		res29[k] = _mm_loadu_si128((const __m128i*)&pred[29 * i_pred + offset]);
		res30[k] = _mm_loadu_si128((const __m128i*)&pred[30 * i_pred + offset]);
		res31[k] = _mm_loadu_si128((const __m128i*)&pred[31 * i_pred + offset]);
	}
	...
	for (k = 0; k < 4; k++)
	{
		res00[k] = _mm_min_epi16(res00[k], max_val);
		res01[k] = _mm_min_epi16(res01[k], max_val);
		res02[k] = _mm_min_epi16(res02[k], max_val);
		res03[k] = _mm_min_epi16(res03[k], max_val);
		res04[k] = _mm_min_epi16(res04[k], max_val);
		res05[k] = _mm_min_epi16(res05[k], max_val);
		res06[k] = _mm_min_epi16(res06[k], max_val);
		res07[k] = _mm_min_epi16(res07[k], max_val);
		res08[k] = _mm_min_epi16(res08[k], max_val);
		res09[k] = _mm_min_epi16(res09[k], max_val);
		res10[k] = _mm_min_epi16(res10[k], max_val);
		res11[k] = _mm_min_epi16(res11[k], max_val);
		res12[k] = _mm_min_epi16(res12[k], max_val);
		res13[k] = _mm_min_epi16(res13[k], max_val);
		res14[k] = _mm_min_epi16(res14[k], max_val);
		res15[k] = _mm_min_epi16(res15[k], max_val);
		res16[k] = _mm_min_epi16(res16[k], max_val);
		res17[k] = _mm_min_epi16(res17[k], max_val);
		res18[k] = _mm_min_epi16(res18[k], max_val);
		res19[k] = _mm_min_epi16(res19[k], max_val);
		res20[k] = _mm_min_epi16(res20[k], max_val);
		res21[k] = _mm_min_epi16(res21[k], max_val);
		res22[k] = _mm_min_epi16(res22[k], max_val);
		res23[k] = _mm_min_epi16(res23[k], max_val);
		res24[k] = _mm_min_epi16(res24[k], max_val);
		res25[k] = _mm_min_epi16(res25[k], max_val);
		res26[k] = _mm_min_epi16(res26[k], max_val);
		res27[k] = _mm_min_epi16(res27[k], max_val);
		res28[k] = _mm_min_epi16(res28[k], max_val);
		res29[k] = _mm_min_epi16(res29[k], max_val);
		res30[k] = _mm_min_epi16(res30[k], max_val);
		res31[k] = _mm_min_epi16(res31[k], max_val);

		res00[k] = _mm_max_epi16(res00[k], zero);
		res01[k] = _mm_max_epi16(res01[k], zero);
		res02[k] = _mm_max_epi16(res02[k], zero);
		res03[k] = _mm_max_epi16(res03[k], zero);
		res04[k] = _mm_max_epi16(res04[k], zero);
		res05[k] = _mm_max_epi16(res05[k], zero);
		res06[k] = _mm_max_epi16(res06[k], zero);
		res07[k] = _mm_max_epi16(res07[k], zero);
		res08[k] = _mm_max_epi16(res08[k], zero);
		res09[k] = _mm_max_epi16(res09[k], zero);
		res10[k] = _mm_max_epi16(res10[k], zero);
		res11[k] = _mm_max_epi16(res11[k], zero);
		res12[k] = _mm_max_epi16(res12[k], zero);
		res13[k] = _mm_max_epi16(res13[k], zero);
		res14[k] = _mm_max_epi16(res14[k], zero);
		res15[k] = _mm_max_epi16(res15[k], zero);
		res16[k] = _mm_max_epi16(res16[k], zero);
		res17[k] = _mm_max_epi16(res17[k], zero);
		res18[k] = _mm_max_epi16(res18[k], zero);
		res19[k] = _mm_max_epi16(res19[k], zero);
		res20[k] = _mm_max_epi16(res20[k], zero);
		res21[k] = _mm_max_epi16(res21[k], zero);
		res22[k] = _mm_max_epi16(res22[k], zero);
		res23[k] = _mm_max_epi16(res23[k], zero);
		res24[k] = _mm_max_epi16(res24[k], zero);
		res25[k] = _mm_max_epi16(res25[k], zero);
		res26[k] = _mm_max_epi16(res26[k], zero);
		res27[k] = _mm_max_epi16(res27[k], zero);
		res28[k] = _mm_max_epi16(res28[k], zero);
		res29[k] = _mm_max_epi16(res29[k], zero);
		res30[k] = _mm_max_epi16(res30[k], zero);
		res31[k] = _mm_max_epi16(res31[k], zero);
	}
	...
	{
		int offset = k << 3;
		_mm_storeu_si128((__m128i*)&dst[0 + offset], res00[k]);
		_mm_storeu_si128((__m128i*)&dst[1 * i_dst + offset], res01[k]);
		_mm_storeu_si128((__m128i*)&dst[2 * i_dst + offset], res02[k]);
		_mm_storeu_si128((__m128i*)&dst[3 * i_dst + offset], res03[k]);
		_mm_storeu_si128((__m128i*)&dst[4 * i_dst + offset], res04[k]);
		_mm_storeu_si128((__m128i*)&dst[5 * i_dst + offset], res05[k]);
		_mm_storeu_si128((__m128i*)&dst[6 * i_dst + offset], res06[k]);
		_mm_storeu_si128((__m128i*)&dst[7 * i_dst + offset], res07[k]);
		_mm_storeu_si128((__m128i*)&dst[8 * i_dst + offset], res08[k]);
		_mm_storeu_si128((__m128i*)&dst[9 * i_dst + offset], res09[k]);

		_mm_storeu_si128((__m128i*)&dst[10 * i_dst + offset], res10[k]);
		_mm_storeu_si128((__m128i*)&dst[11 * i_dst + offset], res11[k]);
		_mm_storeu_si128((__m128i*)&dst[12 * i_dst + offset], res12[k]);
		_mm_storeu_si128((__m128i*)&dst[13 * i_dst + offset], res13[k]);
		_mm_storeu_si128((__m128i*)&dst[14 * i_dst + offset], res14[k]);
		_mm_storeu_si128((__m128i*)&dst[15 * i_dst + offset], res15[k]);
		_mm_storeu_si128((__m128i*)&dst[16 * i_dst + offset], res16[k]);
		_mm_storeu_si128((__m128i*)&dst[17 * i_dst + offset], res17[k]);
		_mm_storeu_si128((__m128i*)&dst[18 * i_dst + offset], res18[k]);
		_mm_storeu_si128((__m128i*)&dst[19 * i_dst + offset], res19[k]);

		_mm_storeu_si128((__m128i*)&dst[20 * i_dst + offset], res20[k]);
		_mm_storeu_si128((__m128i*)&dst[21 * i_dst + offset], res21[k]);
		_mm_storeu_si128((__m128i*)&dst[22 * i_dst + offset], res22[k]);
		_mm_storeu_si128((__m128i*)&dst[23 * i_dst + offset], res23[k]);
		_mm_storeu_si128((__m128i*)&dst[24 * i_dst + offset], res24[k]);
		_mm_storeu_si128((__m128i*)&dst[25 * i_dst + offset], res25[k]);
		_mm_storeu_si128((__m128i*)&dst[26 * i_dst + offset], res26[k]);
		_mm_storeu_si128((__m128i*)&dst[27 * i_dst + offset], res27[k]);
		_mm_storeu_si128((__m128i*)&dst[28 * i_dst + offset], res28[k]);
		_mm_storeu_si128((__m128i*)&dst[29 * i_dst + offset], res29[k]);

		_mm_storeu_si128((__m128i*)&dst[30 * i_dst + offset], res30[k]);
		_mm_storeu_si128((__m128i*)&dst[31 * i_dst + offset], res31[k]);
	}
...
void add_inv_trans_64x64_10bit_sse128(coef_t *src, pel_t *pred, int i_pred, pel_t *dst, int i_dst, int bit_depth)
{
	...
	short vmax = (1 << (bit_depth + 1)) - 1;
	short vmin = -vmax - 1;
	int shift2 = 19 - bit_depth;
	int add2 = 1 << (18 - bit_depth);
	short vmax2 = (1 << (bit_depth)) - 1;
	__m128i max_val, min_val;
...
#undef TRANSPOSE_16x16_16BIT
	...
	{
		int offset = (k << 3);
		V00[k] = _mm_loadu_si128((const __m128i*)&pred[0 + offset]);
		V01[k] = _mm_loadu_si128((const __m128i*)&pred[1 * i_pred + offset]);
		V02[k] = _mm_loadu_si128((const __m128i*)&pred[2 * i_pred + offset]);
		V03[k] = _mm_loadu_si128((const __m128i*)&pred[3 * i_pred + offset]);
		V04[k] = _mm_loadu_si128((const __m128i*)&pred[4 * i_pred + offset]);
		V05[k] = _mm_loadu_si128((const __m128i*)&pred[5 * i_pred + offset]);
		V06[k] = _mm_loadu_si128((const __m128i*)&pred[6 * i_pred + offset]);
		V07[k] = _mm_loadu_si128((const __m128i*)&pred[7 * i_pred + offset]);
		V08[k] = _mm_loadu_si128((const __m128i*)&pred[8 * i_pred + offset]);
		V09[k] = _mm_loadu_si128((const __m128i*)&pred[9 * i_pred + offset]);
		V10[k] = _mm_loadu_si128((const __m128i*)&pred[10 * i_pred + offset]);
		V11[k] = _mm_loadu_si128((const __m128i*)&pred[11 * i_pred + offset]);
		V12[k] = _mm_loadu_si128((const __m128i*)&pred[12 * i_pred + offset]);
		V13[k] = _mm_loadu_si128((const __m128i*)&pred[13 * i_pred + offset]);
		V14[k] = _mm_loadu_si128((const __m128i*)&pred[14 * i_pred + offset]);
		V15[k] = _mm_loadu_si128((const __m128i*)&pred[15 * i_pred + offset]);
		V16[k] = _mm_loadu_si128((const __m128i*)&pred[16 * i_pred + offset]);
		V17[k] = _mm_loadu_si128((const __m128i*)&pred[17 * i_pred + offset]);
		V18[k] = _mm_loadu_si128((const __m128i*)&pred[18 * i_pred + offset]);
		V19[k] = _mm_loadu_si128((const __m128i*)&pred[19 * i_pred + offset]);
		V20[k] = _mm_loadu_si128((const __m128i*)&pred[20 * i_pred + offset]);
		V21[k] = _mm_loadu_si128((const __m128i*)&pred[21 * i_pred + offset]);
		V22[k] = _mm_loadu_si128((const __m128i*)&pred[22 * i_pred + offset]);
		V23[k] = _mm_loadu_si128((const __m128i*)&pred[23 * i_pred + offset]);
		V24[k] = _mm_loadu_si128((const __m128i*)&pred[24 * i_pred + offset]);
		V25[k] = _mm_loadu_si128((const __m128i*)&pred[25 * i_pred + offset]);
		V26[k] = _mm_loadu_si128((const __m128i*)&pred[26 * i_pred + offset]);
		V27[k] = _mm_loadu_si128((const __m128i*)&pred[27 * i_pred + offset]);
		V28[k] = _mm_loadu_si128((const __m128i*)&pred[28 * i_pred + offset]);
		V29[k] = _mm_loadu_si128((const __m128i*)&pred[29 * i_pred + offset]);
		V30[k] = _mm_loadu_si128((const __m128i*)&pred[30 * i_pred + offset]);
		V31[k] = _mm_loadu_si128((const __m128i*)&pred[31 * i_pred + offset]);
		V32[k] = _mm_loadu_si128((const __m128i*)&pred[32 * i_pred + offset]);
		V33[k] = _mm_loadu_si128((const __m128i*)&pred[33 * i_pred + offset]);
		V34[k] = _mm_loadu_si128((const __m128i*)&pred[34 * i_pred + offset]);
		V35[k] = _mm_loadu_si128((const __m128i*)&pred[35 * i_pred + offset]);
		V36[k] = _mm_loadu_si128((const __m128i*)&pred[36 * i_pred + offset]);
		V37[k] = _mm_loadu_si128((const __m128i*)&pred[37 * i_pred + offset]);
		V38[k] = _mm_loadu_si128((const __m128i*)&pred[38 * i_pred + offset]);
		V39[k] = _mm_loadu_si128((const __m128i*)&pred[39 * i_pred + offset]);
		V40[k] = _mm_loadu_si128((const __m128i*)&pred[40 * i_pred + offset]);
		V41[k] = _mm_loadu_si128((const __m128i*)&pred[41 * i_pred + offset]);
		V42[k] = _mm_loadu_si128((const __m128i*)&pred[42 * i_pred + offset]);
		V43[k] = _mm_loadu_si128((const __m128i*)&pred[43 * i_pred + offset]);
		V44[k] = _mm_loadu_si128((const __m128i*)&pred[44 * i_pred + offset]);
		V45[k] = _mm_loadu_si128((const __m128i*)&pred[45 * i_pred + offset]);
		V46[k] = _mm_loadu_si128((const __m128i*)&pred[46 * i_pred + offset]);
		V47[k] = _mm_loadu_si128((const __m128i*)&pred[47 * i_pred + offset]);
		V48[k] = _mm_loadu_si128((const __m128i*)&pred[48 * i_pred + offset]);
		V49[k] = _mm_loadu_si128((const __m128i*)&pred[49 * i_pred + offset]);
		V50[k] = _mm_loadu_si128((const __m128i*)&pred[50 * i_pred + offset]);
		V51[k] = _mm_loadu_si128((const __m128i*)&pred[51 * i_pred + offset]);
		V52[k] = _mm_loadu_si128((const __m128i*)&pred[52 * i_pred + offset]);
		V53[k] = _mm_loadu_si128((const __m128i*)&pred[53 * i_pred + offset]);
		V54[k] = _mm_loadu_si128((const __m128i*)&pred[54 * i_pred + offset]);
		V55[k] = _mm_loadu_si128((const __m128i*)&pred[55 * i_pred + offset]);
		V56[k] = _mm_loadu_si128((const __m128i*)&pred[56 * i_pred + offset]);
		V57[k] = _mm_loadu_si128((const __m128i*)&pred[57 * i_pred + offset]);
		V58[k] = _mm_loadu_si128((const __m128i*)&pred[58 * i_pred + offset]);
		V59[k] = _mm_loadu_si128((const __m128i*)&pred[59 * i_pred + offset]);
		V60[k] = _mm_loadu_si128((const __m128i*)&pred[60 * i_pred + offset]);
		V61[k] = _mm_loadu_si128((const __m128i*)&pred[61 * i_pred + offset]);
		V62[k] = _mm_loadu_si128((const __m128i*)&pred[62 * i_pred + offset]);
		V63[k] = _mm_loadu_si128((const __m128i*)&pred[63 * i_pred + offset]);
	}
	...
	max_val = _mm_set1_epi16(vmax2);
	for (k = 0; k < 8; k ++)
	{
		V00[k] = _mm_min_epi16(max_val, T00[k]);
		V01[k] = _mm_min_epi16(max_val, T01[k]);
		V02[k] = _mm_min_epi16(max_val, T02[k]);
		V03[k] = _mm_min_epi16(max_val, T03[k]);
		V04[k] = _mm_min_epi16(max_val, T04[k]);
		V05[k] = _mm_min_epi16(max_val, T05[k]);
		V06[k] = _mm_min_epi16(max_val, T06[k]);
		V07[k] = _mm_min_epi16(max_val, T07[k]);
		V08[k] = _mm_min_epi16(max_val, T08[k]);
		V09[k] = _mm_min_epi16(max_val, T09[k]);
		V10[k] = _mm_min_epi16(max_val, T10[k]);
		V11[k] = _mm_min_epi16(max_val, T11[k]);
		V12[k] = _mm_min_epi16(max_val, T12[k]);
		V13[k] = _mm_min_epi16(max_val, T13[k]);
		V14[k] = _mm_min_epi16(max_val, T14[k]);
		V15[k] = _mm_min_epi16(max_val, T15[k]);
		V16[k] = _mm_min_epi16(max_val, T16[k]);
		V17[k] = _mm_min_epi16(max_val, T17[k]);
		V18[k] = _mm_min_epi16(max_val, T18[k]);
		V19[k] = _mm_min_epi16(max_val, T19[k]);
		V20[k] = _mm_min_epi16(max_val, T20[k]);
		V21[k] = _mm_min_epi16(max_val, T21[k]);
		V22[k] = _mm_min_epi16(max_val, T22[k]);
		V23[k] = _mm_min_epi16(max_val, T23[k]);
		V24[k] = _mm_min_epi16(max_val, T24[k]);
		V25[k] = _mm_min_epi16(max_val, T25[k]);
		V26[k] = _mm_min_epi16(max_val, T26[k]);
		V27[k] = _mm_min_epi16(max_val, T27[k]);
		V28[k] = _mm_min_epi16(max_val, T28[k]);
		V29[k] = _mm_min_epi16(max_val, T29[k]);
		V30[k] = _mm_min_epi16(max_val, T30[k]);
		V31[k] = _mm_min_epi16(max_val, T31[k]);
		V32[k] = _mm_min_epi16(max_val, T32[k]);
		V33[k] = _mm_min_epi16(max_val, T33[k]);
		V34[k] = _mm_min_epi16(max_val, T34[k]);
		V35[k] = _mm_min_epi16(max_val, T35[k]);
		V36[k] = _mm_min_epi16(max_val, T36[k]);
		V37[k] = _mm_min_epi16(max_val, T37[k]);
		V38[k] = _mm_min_epi16(max_val, T38[k]);
		V39[k] = _mm_min_epi16(max_val, T39[k]);
		V40[k] = _mm_min_epi16(max_val, T40[k]);
		V41[k] = _mm_min_epi16(max_val, T41[k]);
		V42[k] = _mm_min_epi16(max_val, T42[k]);
		V43[k] = _mm_min_epi16(max_val, T43[k]);
		V44[k] = _mm_min_epi16(max_val, T44[k]);
		V45[k] = _mm_min_epi16(max_val, T45[k]);
		V46[k] = _mm_min_epi16(max_val, T46[k]);
		V47[k] = _mm_min_epi16(max_val, T47[k]);
		V48[k] = _mm_min_epi16(max_val, T48[k]);
		V49[k] = _mm_min_epi16(max_val, T49[k]);
		V50[k] = _mm_min_epi16(max_val, T50[k]);
		V51[k] = _mm_min_epi16(max_val, T51[k]);
		V52[k] = _mm_min_epi16(max_val, T52[k]);
		V53[k] = _mm_min_epi16(max_val, T53[k]);
		V54[k] = _mm_min_epi16(max_val, T54[k]);
		V55[k] = _mm_min_epi16(max_val, T55[k]);
		V56[k] = _mm_min_epi16(max_val, T56[k]);
		V57[k] = _mm_min_epi16(max_val, T57[k]);
		V58[k] = _mm_min_epi16(max_val, T58[k]);
		V59[k] = _mm_min_epi16(max_val, T59[k]);
		V60[k] = _mm_min_epi16(max_val, T60[k]);
		V61[k] = _mm_min_epi16(max_val, T61[k]);
		V62[k] = _mm_min_epi16(max_val, T62[k]);
		V63[k] = _mm_min_epi16(max_val, T63[k]);


		V00[k] = _mm_max_epi16(zero, V00[k]);
		V01[k] = _mm_max_epi16(zero, V01[k]);
		V02[k] = _mm_max_epi16(zero, V02[k]);
		V03[k] = _mm_max_epi16(zero, V03[k]);
		V04[k] = _mm_max_epi16(zero, V04[k]);
		V05[k] = _mm_max_epi16(zero, V05[k]);
		V06[k] = _mm_max_epi16(zero, V06[k]);
		V07[k] = _mm_max_epi16(zero, V07[k]);
		V08[k] = _mm_max_epi16(zero, V08[k]);
		V09[k] = _mm_max_epi16(zero, V09[k]);
		V10[k] = _mm_max_epi16(zero, V10[k]);
		V11[k] = _mm_max_epi16(zero, V11[k]);
		V12[k] = _mm_max_epi16(zero, V12[k]);
		V13[k] = _mm_max_epi16(zero, V13[k]);
		V14[k] = _mm_max_epi16(zero, V14[k]);
		V15[k] = _mm_max_epi16(zero, V15[k]);
		V16[k] = _mm_max_epi16(zero, V16[k]);
		V17[k] = _mm_max_epi16(zero, V17[k]);
		V18[k] = _mm_max_epi16(zero, V18[k]);
		V19[k] = _mm_max_epi16(zero, V19[k]);
		V20[k] = _mm_max_epi16(zero, V20[k]);
		V21[k] = _mm_max_epi16(zero, V21[k]);
		V22[k] = _mm_max_epi16(zero, V22[k]);
		V23[k] = _mm_max_epi16(zero, V23[k]);
		V24[k] = _mm_max_epi16(zero, V24[k]);
		V25[k] = _mm_max_epi16(zero, V25[k]);
		V26[k] = _mm_max_epi16(zero, V26[k]);
		V27[k] = _mm_max_epi16(zero, V27[k]);
		V28[k] = _mm_max_epi16(zero, V28[k]);
		V29[k] = _mm_max_epi16(zero, V29[k]);
		V30[k] = _mm_max_epi16(zero, V30[k]);
		V31[k] = _mm_max_epi16(zero, V31[k]);
		V32[k] = _mm_max_epi16(zero, V32[k]);
		V33[k] = _mm_max_epi16(zero, V33[k]);
		V34[k] = _mm_max_epi16(zero, V34[k]);
		V35[k] = _mm_max_epi16(zero, V35[k]);
		V36[k] = _mm_max_epi16(zero, V36[k]);
		V37[k] = _mm_max_epi16(zero, V37[k]);
		V38[k] = _mm_max_epi16(zero, V38[k]);
		V39[k] = _mm_max_epi16(zero, V39[k]);
		V40[k] = _mm_max_epi16(zero, V40[k]);
		V41[k] = _mm_max_epi16(zero, V41[k]);
		V42[k] = _mm_max_epi16(zero, V42[k]);
		V43[k] = _mm_max_epi16(zero, V43[k]);
		V44[k] = _mm_max_epi16(zero, V44[k]);
		V45[k] = _mm_max_epi16(zero, V45[k]);
		V46[k] = _mm_max_epi16(zero, V46[k]);
		V47[k] = _mm_max_epi16(zero, V47[k]);
		V48[k] = _mm_max_epi16(zero, V48[k]);
		V49[k] = _mm_max_epi16(zero, V49[k]);
		V50[k] = _mm_max_epi16(zero, V50[k]);
		V51[k] = _mm_max_epi16(zero, V51[k]);
		V52[k] = _mm_max_epi16(zero, V52[k]);
		V53[k] = _mm_max_epi16(zero, V53[k]);
		V54[k] = _mm_max_epi16(zero, V54[k]);
		V55[k] = _mm_max_epi16(zero, V55[k]);
		V56[k] = _mm_max_epi16(zero, V56[k]);
		V57[k] = _mm_max_epi16(zero, V57[k]);
		V58[k] = _mm_max_epi16(zero, V58[k]);
		V59[k] = _mm_max_epi16(zero, V59[k]);
		V60[k] = _mm_max_epi16(zero, V60[k]);
		V61[k] = _mm_max_epi16(zero, V61[k]);
		V62[k] = _mm_max_epi16(zero, V62[k]);
		V63[k] = _mm_max_epi16(zero, V63[k]);
	}
...
#define COMPUTE_ROW(r0103, r0507, r0911, r1315, c0103, c0507, c0911, c1315, row) \
	...
	row = _mm_add_epi32(T00, T01);
			...
			COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_p45_p45, c16_p43_p44, c16_p39_p41, c16_p34_p36, O00A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_p41_p45, c16_p23_p34, c16_n02_p11, c16_n27_n15, O01A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_p34_p44, c16_n07_p15, c16_n41_n27, c16_n39_n45, O02A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_p23_p43, c16_n34_n07, c16_n36_n45, c16_p19_n11, O03A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_p11_p41, c16_n45_n27, c16_p07_n30, c16_p43_p39, O04A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n02_p39, c16_n36_n41, c16_p43_p07, c16_n11_p34, O05A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n15_p36, c16_n11_n45, c16_p34_p39, c16_n45_n19, O06A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n27_p34, c16_p19_n39, c16_n11_p43, c16_p02_n45, O07A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n36_p30, c16_p41_n23, c16_n44_p15, c16_p45_n07, O08A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n43_p27, c16_p44_n02, c16_n30_n23, c16_p07_p41, O09A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n45_p23, c16_p27_p19, c16_p15_n45, c16_n44_p30, O10A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n44_p19, c16_n02_p36, c16_p45_n34, c16_n15_n23, O11A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n39_p15, c16_n30_p45, c16_p27_p02, c16_p41_n44, O12A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n30_p11, c16_n45_p43, c16_n19_p36, c16_p23_n02, O13A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n19_p07, c16_n39_p30, c16_n45_p44, c16_n36_p43, O14A)
				COMPUTE_ROW(T_00_00A, T_00_01A, T_00_02A, T_00_03A, \
				c16_n07_p02, c16_n15_p11, c16_n23_p19, c16_n30_p27, O15A)

				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_p45_p45, c16_p43_p44, c16_p39_p41, c16_p34_p36, O00B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_p41_p45, c16_p23_p34, c16_n02_p11, c16_n27_n15, O01B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_p34_p44, c16_n07_p15, c16_n41_n27, c16_n39_n45, O02B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_p23_p43, c16_n34_n07, c16_n36_n45, c16_p19_n11, O03B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_p11_p41, c16_n45_n27, c16_p07_n30, c16_p43_p39, O04B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n02_p39, c16_n36_n41, c16_p43_p07, c16_n11_p34, O05B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n15_p36, c16_n11_n45, c16_p34_p39, c16_n45_n19, O06B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n27_p34, c16_p19_n39, c16_n11_p43, c16_p02_n45, O07B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n36_p30, c16_p41_n23, c16_n44_p15, c16_p45_n07, O08B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n43_p27, c16_p44_n02, c16_n30_n23, c16_p07_p41, O09B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n45_p23, c16_p27_p19, c16_p15_n45, c16_n44_p30, O10B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n44_p19, c16_n02_p36, c16_p45_n34, c16_n15_n23, O11B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n39_p15, c16_n30_p45, c16_p27_p02, c16_p41_n44, O12B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n30_p11, c16_n45_p43, c16_n19_p36, c16_p23_n02, O13B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n19_p07, c16_n39_p30, c16_n45_p44, c16_n36_p43, O14B)
				COMPUTE_ROW(T_00_00B, T_00_01B, T_00_02B, T_00_03B, \
				c16_n07_p02, c16_n15_p11, c16_n23_p19, c16_n30_p27, O15B)
...
#undef COMPUTE_ROW
		...
		{
#define COMPUTE_ROW(row0206, row1014, c0206, c1014, row) \
	row = _mm_add_epi32(_mm_madd_epi16(row0206, c0206), _mm_madd_epi16(row1014, c1014));

			COMPUTE_ROW(T_00_08A, T_00_09A, c16_p43_p45, c16_p35_p40, EO0A)
				COMPUTE_ROW(T_00_08A, T_00_09A, c16_p29_p43, c16_n21_p04, EO1A)
				COMPUTE_ROW(T_00_08A, T_00_09A, c16_p04_p40, c16_n43_n35, EO2A)
				COMPUTE_ROW(T_00_08A, T_00_09A, c16_n21_p35, c16_p04_n43, EO3A)
				COMPUTE_ROW(T_00_08A, T_00_09A, c16_n40_p29, c16_p45_n13, EO4A)
				COMPUTE_ROW(T_00_08A, T_00_09A, c16_n45_p21, c16_p13_p29, EO5A)
				COMPUTE_ROW(T_00_08A, T_00_09A, c16_n35_p13, c16_n40_p45, EO6A)
				COMPUTE_ROW(T_00_08A, T_00_09A, c16_n13_p04, c16_n29_p21, EO7A)

				COMPUTE_ROW(T_00_08B, T_00_09B, c16_p43_p45, c16_p35_p40, EO0B)
				COMPUTE_ROW(T_00_08B, T_00_09B, c16_p29_p43, c16_n21_p04, EO1B)
				COMPUTE_ROW(T_00_08B, T_00_09B, c16_p04_p40, c16_n43_n35, EO2B)
				COMPUTE_ROW(T_00_08B, T_00_09B, c16_n21_p35, c16_p04_n43, EO3B)
				COMPUTE_ROW(T_00_08B, T_00_09B, c16_n40_p29, c16_p45_n13, EO4B)
				COMPUTE_ROW(T_00_08B, T_00_09B, c16_n45_p21, c16_p13_p29, EO5B)
				COMPUTE_ROW(T_00_08B, T_00_09B, c16_n35_p13, c16_n40_p45, EO6B)
				COMPUTE_ROW(T_00_08B, T_00_09B, c16_n13_p04, c16_n29_p21, EO7B)
#undef COMPUTE_ROW
		}
		{
...
#undef TRANSPOSE_8x8_16BIT
	...
	//2nd DCT
	c32_rnd = _mm_set1_epi32(add2);				//第二次四舍五入
	nShift = shift2;
...
void xTr2nd_8_1d_Inv_Hor_10bit_sse128(coef_t *src, int i_src)
{
	__m128i	tmpCoef0, tmpCoef1, tmpCoef2, tmpCoef3;
...
void xTr2nd_8_1d_Inv_Vert_10bit_sse128(coef_t *src, int i_src)
{
	__m128i	tmpSrc0, tmpSrc1, tmpSrc2, tmpSrc3, tmpLoad0, tmpLoad1, tmpLoad2, tmpLoad3;
	__m128i tmpC0, tmpC1, tmpC2, tmpC3;
...
void add_inv_trans_16x16_10bit_sse256(coef_t *src, pel_t *pred, int i_pred, pel_t *dst, int i_dst, int bit_depth)
{
	...
	__m256i in00, in01, in02, in03, in04, in05, in06, in07, in08, in09, in10, in11, in12, in13, in14, in15;
	__m256i im00, im01, im02, im03, im04, im05, im06, im07, im08, im09, im10, im11, im12, im13, im14, im15;
...
#undef TRANSPOSE_16x16_16BIT
	...
	//The second IDCT
	{
		...
		c32_rnd = _mm256_set1_epi32(add2);				// pass == 1 第二次四舍五入
		nShift = shift2;
...
#undef TRANSPOSE_16x16_16BIT
	...
	//
	im00 = _mm256_permute2x128_si256(in00, in01, 0x20);
	im01 = _mm256_permute2x128_si256(in00, in01, 0x31);
	im02 = _mm256_permute2x128_si256(in02, in03, 0x20);
	im03 = _mm256_permute2x128_si256(in02, in03, 0x31);
	im04 = _mm256_permute2x128_si256(in04, in05, 0x20);
	im05 = _mm256_permute2x128_si256(in04, in05, 0x31);
	im06 = _mm256_permute2x128_si256(in06, in07, 0x20);
	im07 = _mm256_permute2x128_si256(in06, in07, 0x31);
	im08 = _mm256_permute2x128_si256(in08, in09, 0x20);
	im09 = _mm256_permute2x128_si256(in08, in09, 0x31);
	im10 = _mm256_permute2x128_si256(in10, in11, 0x20);
	im11 = _mm256_permute2x128_si256(in10, in11, 0x31);
	im12 = _mm256_permute2x128_si256(in12, in13, 0x20);
	im13 = _mm256_permute2x128_si256(in12, in13, 0x31);
	im14 = _mm256_permute2x128_si256(in14, in15, 0x20);
	im15 = _mm256_permute2x128_si256(in14, in15, 0x31);

	//clip
	max_val = _mm256_set1_epi16(vmax);
	min_val = _mm256_set1_epi16(vmin);
	in00 = _mm256_min_epi16(im00, max_val);
	in00 = _mm256_max_epi16(in00, min_val);
	in01 = _mm256_min_epi16(im01, max_val);
	in01 = _mm256_max_epi16(in01, min_val);
	in02 = _mm256_min_epi16(im02, max_val);
	in02 = _mm256_max_epi16(in02, min_val);
	in03 = _mm256_min_epi16(im03, max_val);
	in03 = _mm256_max_epi16(in03, min_val);
	in04 = _mm256_min_epi16(im04, max_val);
	in04 = _mm256_max_epi16(in04, min_val);
	in05 = _mm256_min_epi16(im05, max_val);
	in05 = _mm256_max_epi16(in05, min_val);
	in06 = _mm256_min_epi16(im06, max_val);
	in06 = _mm256_max_epi16(in06, min_val);
	in07 = _mm256_min_epi16(im07, max_val);
	in07 = _mm256_max_epi16(in07, min_val);
	in08 = _mm256_min_epi16(im08, max_val);
	in08 = _mm256_max_epi16(in08, min_val);
	in09 = _mm256_min_epi16(im09, max_val);
	in09 = _mm256_max_epi16(in09, min_val);
	in10 = _mm256_min_epi16(im10, max_val);
	in10 = _mm256_max_epi16(in10, min_val);
	in11 = _mm256_min_epi16(im11, max_val);
	in11 = _mm256_max_epi16(in11, min_val);
	in12 = _mm256_min_epi16(im12, max_val);
	in12 = _mm256_max_epi16(in12, min_val);
	in13 = _mm256_min_epi16(im13, max_val);
	in13 = _mm256_max_epi16(in13, min_val);
	in14 = _mm256_min_epi16(im14, max_val);
	in14 = _mm256_max_epi16(in14, min_val);
	in15 = _mm256_min_epi16(im15, max_val);
	in15 = _mm256_max_epi16(in15, min_val);

	//load & add

	res00 = _mm256_loadu_si256((__m256i*)&pred[0]);
	res01 = _mm256_loadu_si256((__m256i*)&pred[8 * i_pred]);
	res02 = _mm256_loadu_si256((__m256i*)&pred[1 * i_pred]);
	res03 = _mm256_loadu_si256((__m256i*)&pred[9 * i_pred]);
	res04 = _mm256_loadu_si256((__m256i*)&pred[2 * i_pred]);
	res05 = _mm256_loadu_si256((__m256i*)&pred[10 * i_pred]);
	res06 = _mm256_loadu_si256((__m256i*)&pred[3 * i_pred]);
	res07 = _mm256_loadu_si256((__m256i*)&pred[11 * i_pred]);
	res08 = _mm256_loadu_si256((__m256i*)&pred[4 * i_pred]);
	res09 = _mm256_loadu_si256((__m256i*)&pred[12 * i_pred]);
	res10 = _mm256_loadu_si256((__m256i*)&pred[5 * i_pred]);
	res11 = _mm256_loadu_si256((__m256i*)&pred[13 * i_pred]);
	res12 = _mm256_loadu_si256((__m256i*)&pred[6 * i_pred]);
	res13 = _mm256_loadu_si256((__m256i*)&pred[14 * i_pred]);
	res14 = _mm256_loadu_si256((__m256i*)&pred[7 * i_pred]);
	res15 = _mm256_loadu_si256((__m256i*)&pred[15 * i_pred]);
	...
	_mm256_storeu_si256((__m256i*)&dst[0], res00); 
	_mm256_storeu_si256((__m256i*)&dst[8 * i_dst], res01);
	_mm256_storeu_si256((__m256i*)&dst[1 * i_dst], res02);
	_mm256_storeu_si256((__m256i*)&dst[9 * i_dst], res03);
	_mm256_storeu_si256((__m256i*)&dst[2 * i_dst], res04);
	_mm256_storeu_si256((__m256i*)&dst[10 * i_dst], res05);
	_mm256_storeu_si256((__m256i*)&dst[3 * i_dst], res06);
	_mm256_storeu_si256((__m256i*)&dst[11 * i_dst], res07);
	_mm256_storeu_si256((__m256i*)&dst[4 * i_dst], res08);
	_mm256_storeu_si256((__m256i*)&dst[12 * i_dst], res09);
	_mm256_storeu_si256((__m256i*)&dst[5 * i_dst], res10);
	_mm256_storeu_si256((__m256i*)&dst[13 * i_dst], res11);
	_mm256_storeu_si256((__m256i*)&dst[6 * i_dst], res12);
	_mm256_storeu_si256((__m256i*)&dst[14 * i_dst], res13);
	_mm256_storeu_si256((__m256i*)&dst[7 * i_dst], res14);
	_mm256_storeu_si256((__m256i*)&dst[15 * i_dst], res15);
}
...
void add_inv_trans_32x32_10bit_sse256(coef_t *src, pel_t *pred, int i_pred, pel_t *dst, int i_dst, int bit_depth)
{
	...
	// DCT1
	__m256i in00[2], in01[2], in02[2], in03[2], in04[2], in05[2], in06[2], in07[2], in08[2], in09[2], in10[2], in11[2], in12[2], in13[2], in14[2], in15[2], in16[2], in17[2], in18[2], in19[2], in20[2], in21[2], in22[2], in23[2], in24[2], in25[2], in26[2], in27[2], in28[2], in29[2], in30[2], in31[2];
	__m256i im00[2], im01[2], im02[2], im03[2], im04[2], im05[2], im06[2], im07[2], im08[2], im09[2], im10[2], im11[2], im12[2], im13[2], im14[2], im15[2], im16[2], im17[2], im18[2], im19[2], im20[2], im21[2], im22[2], im23[2], im24[2], im25[2], im26[2], im27[2], im28[2], im29[2], im30[2], im31[2];
...
#undef TRANSPOSE_16x16_16BIT
	...
	for (k = 0; k < 2; k++){
		//0~15
		im00[k] = _mm256_permute2x128_si256(in00[k], in01[k], 0x20);
		im01[k] = _mm256_permute2x128_si256(in00[k], in01[k], 0x31);
		im02[k] = _mm256_permute2x128_si256(in02[k], in03[k], 0x20);
		im03[k] = _mm256_permute2x128_si256(in02[k], in03[k], 0x31);
		im04[k] = _mm256_permute2x128_si256(in04[k], in05[k], 0x20);
		im05[k] = _mm256_permute2x128_si256(in04[k], in05[k], 0x31);
		im06[k] = _mm256_permute2x128_si256(in06[k], in07[k], 0x20);
		im07[k] = _mm256_permute2x128_si256(in06[k], in07[k], 0x31);
		im08[k] = _mm256_permute2x128_si256(in08[k], in09[k], 0x20);
		im09[k] = _mm256_permute2x128_si256(in08[k], in09[k], 0x31);
		im10[k] = _mm256_permute2x128_si256(in10[k], in11[k], 0x20);
		im11[k] = _mm256_permute2x128_si256(in10[k], in11[k], 0x31);
		im12[k] = _mm256_permute2x128_si256(in12[k], in13[k], 0x20);
		im13[k] = _mm256_permute2x128_si256(in12[k], in13[k], 0x31);
		im14[k] = _mm256_permute2x128_si256(in14[k], in15[k], 0x20);
		im15[k] = _mm256_permute2x128_si256(in14[k], in15[k], 0x31);
		//16~31
		im16[k] = _mm256_permute2x128_si256(in16[k], in17[k], 0x20);
		im17[k] = _mm256_permute2x128_si256(in16[k], in17[k], 0x31);
		im18[k] = _mm256_permute2x128_si256(in18[k], in19[k], 0x20);
		im19[k] = _mm256_permute2x128_si256(in18[k], in19[k], 0x31);
		im20[k] = _mm256_permute2x128_si256(in20[k], in21[k], 0x20);
		im21[k] = _mm256_permute2x128_si256(in20[k], in21[k], 0x31);
		im22[k] = _mm256_permute2x128_si256(in22[k], in23[k], 0x20);
		im23[k] = _mm256_permute2x128_si256(in22[k], in23[k], 0x31);
		im24[k] = _mm256_permute2x128_si256(in24[k], in25[k], 0x20);
		im25[k] = _mm256_permute2x128_si256(in24[k], in25[k], 0x31);
		im26[k] = _mm256_permute2x128_si256(in26[k], in27[k], 0x20);
		im27[k] = _mm256_permute2x128_si256(in26[k], in27[k], 0x31);
		im28[k] = _mm256_permute2x128_si256(in28[k], in29[k], 0x20);
		im29[k] = _mm256_permute2x128_si256(in28[k], in29[k], 0x31);
		im30[k] = _mm256_permute2x128_si256(in30[k], in31[k], 0x20);
		im31[k] = _mm256_permute2x128_si256(in30[k], in31[k], 0x31);
	}
	...
	for (k = 0; k < 2; k++)
	{
		in00[k] = _mm256_min_epi16(im00[k], max_val);
		in00[k] = _mm256_max_epi16(in00[k], min_val);
		in01[k] = _mm256_min_epi16(im01[k], max_val);
		in01[k] = _mm256_max_epi16(in01[k], min_val);
		in02[k] = _mm256_min_epi16(im02[k], max_val);
		in02[k] = _mm256_max_epi16(in02[k], min_val);
		in03[k] = _mm256_min_epi16(im03[k], max_val);
		in03[k] = _mm256_max_epi16(in03[k], min_val);
		in04[k] = _mm256_min_epi16(im04[k], max_val);
		in04[k] = _mm256_max_epi16(in04[k], min_val);
		in05[k] = _mm256_min_epi16(im05[k], max_val);
		in05[k] = _mm256_max_epi16(in05[k], min_val);
		in06[k] = _mm256_min_epi16(im06[k], max_val);
		in06[k] = _mm256_max_epi16(in06[k], min_val);
		in07[k] = _mm256_min_epi16(im07[k], max_val);
		in07[k] = _mm256_max_epi16(in07[k], min_val);
		in08[k] = _mm256_min_epi16(im08[k], max_val);
		in08[k] = _mm256_max_epi16(in08[k], min_val);
		in09[k] = _mm256_min_epi16(im09[k], max_val);
		in09[k] = _mm256_max_epi16(in09[k], min_val);
		in10[k] = _mm256_min_epi16(im10[k], max_val);
		in10[k] = _mm256_max_epi16(in10[k], min_val);
		in11[k] = _mm256_min_epi16(im11[k], max_val);
		in11[k] = _mm256_max_epi16(in11[k], min_val);
		in12[k] = _mm256_min_epi16(im12[k], max_val);
		in12[k] = _mm256_max_epi16(in12[k], min_val);
		in13[k] = _mm256_min_epi16(im13[k], max_val);
		in13[k] = _mm256_max_epi16(in13[k], min_val);
		in14[k] = _mm256_min_epi16(im14[k], max_val);
		in14[k] = _mm256_max_epi16(in14[k], min_val);
		in15[k] = _mm256_min_epi16(im15[k], max_val);
		in15[k] = _mm256_max_epi16(in15[k], min_val);
		in16[k] = _mm256_min_epi16(im16[k], max_val);
		in16[k] = _mm256_max_epi16(in16[k], min_val);
		in17[k] = _mm256_min_epi16(im17[k], max_val);
		in17[k] = _mm256_max_epi16(in17[k], min_val);
		in18[k] = _mm256_min_epi16(im18[k], max_val);
		in18[k] = _mm256_max_epi16(in18[k], min_val);
		in19[k] = _mm256_min_epi16(im19[k], max_val);
		in19[k] = _mm256_max_epi16(in19[k], min_val);
		in20[k] = _mm256_min_epi16(im20[k], max_val);
		in20[k] = _mm256_max_epi16(in20[k], min_val);
		in21[k] = _mm256_min_epi16(im21[k], max_val);
		in21[k] = _mm256_max_epi16(in21[k], min_val);
		in22[k] = _mm256_min_epi16(im22[k], max_val);
		in22[k] = _mm256_max_epi16(in22[k], min_val);
		in23[k] = _mm256_min_epi16(im23[k], max_val);
		in23[k] = _mm256_max_epi16(in23[k], min_val);
		in24[k] = _mm256_min_epi16(im24[k], max_val);
		in24[k] = _mm256_max_epi16(in24[k], min_val);
		in25[k] = _mm256_min_epi16(im25[k], max_val);
		in25[k] = _mm256_max_epi16(in25[k], min_val);
		in26[k] = _mm256_min_epi16(im26[k], max_val);
		in26[k] = _mm256_max_epi16(in26[k], min_val);
		in27[k] = _mm256_min_epi16(im27[k], max_val);
		in27[k] = _mm256_max_epi16(in27[k], min_val);
		in28[k] = _mm256_min_epi16(im28[k], max_val);
		in28[k] = _mm256_max_epi16(in28[k], min_val);
		in29[k] = _mm256_min_epi16(im29[k], max_val);
		in29[k] = _mm256_max_epi16(in29[k], min_val);
		in30[k] = _mm256_min_epi16(im30[k], max_val);
		in30[k] = _mm256_max_epi16(in30[k], min_val);
		in31[k] = _mm256_min_epi16(im31[k], max_val);
	...
	for (k = 0; k < 2; k++)
	{
		res00[k] = _mm256_loadu_si256((const __m256i*)&pred[0 + 16 * k]);
		res02[k] = _mm256_loadu_si256((const __m256i*)&pred[i_pred + 16 * k]);
		res04[k] = _mm256_loadu_si256((const __m256i*)&pred[2 * i_pred + 16 * k]);
		res06[k] = _mm256_loadu_si256((const __m256i*)&pred[3 * i_pred + 16 * k]);
		res08[k] = _mm256_loadu_si256((const __m256i*)&pred[4 * i_pred + 16 * k]);
		res10[k] = _mm256_loadu_si256((const __m256i*)&pred[5 * i_pred + 16 * k]);
		res12[k] = _mm256_loadu_si256((const __m256i*)&pred[6 * i_pred + 16 * k]);
		res14[k] = _mm256_loadu_si256((const __m256i*)&pred[7 * i_pred + 16 * k]);

		res01[k] = _mm256_loadu_si256((const __m256i*)&pred[8 * i_pred + 16 * k]);
		res03[k] = _mm256_loadu_si256((const __m256i*)&pred[9 * i_pred + 16 * k]);
		res05[k] = _mm256_loadu_si256((const __m256i*)&pred[10 * i_pred + 16 * k]);
		res07[k] = _mm256_loadu_si256((const __m256i*)&pred[11 * i_pred + 16 * k]);
		res09[k] = _mm256_loadu_si256((const __m256i*)&pred[12 * i_pred + 16 * k]);
		res11[k] = _mm256_loadu_si256((const __m256i*)&pred[13 * i_pred + 16 * k]);
		res13[k] = _mm256_loadu_si256((const __m256i*)&pred[14 * i_pred + 16 * k]);
		res15[k] = _mm256_loadu_si256((const __m256i*)&pred[15 * i_pred + 16 * k]);

		res16[k] = _mm256_loadu_si256((const __m256i*)&pred[16 * i_pred + 16 * k]);
		res18[k] = _mm256_loadu_si256((const __m256i*)&pred[17 * i_pred + 16 * k]);
		res20[k] = _mm256_loadu_si256((const __m256i*)&pred[18 * i_pred + 16 * k]);
		res22[k] = _mm256_loadu_si256((const __m256i*)&pred[19 * i_pred + 16 * k]);
		res24[k] = _mm256_loadu_si256((const __m256i*)&pred[20 * i_pred + 16 * k]);
		res26[k] = _mm256_loadu_si256((const __m256i*)&pred[21 * i_pred + 16 * k]);
		res28[k] = _mm256_loadu_si256((const __m256i*)&pred[22 * i_pred + 16 * k]);
		res30[k] = _mm256_loadu_si256((const __m256i*)&pred[23 * i_pred + 16 * k]);

		res17[k] = _mm256_loadu_si256((const __m256i*)&pred[24 * i_pred + 16 * k]);
		res19[k] = _mm256_loadu_si256((const __m256i*)&pred[25 * i_pred + 16 * k]);
		res21[k] = _mm256_loadu_si256((const __m256i*)&pred[26 * i_pred + 16 * k]);
		res23[k] = _mm256_loadu_si256((const __m256i*)&pred[27 * i_pred + 16 * k]);
		res25[k] = _mm256_loadu_si256((const __m256i*)&pred[28 * i_pred + 16 * k]);
		res27[k] = _mm256_loadu_si256((const __m256i*)&pred[29 * i_pred + 16 * k]);
		res29[k] = _mm256_loadu_si256((const __m256i*)&pred[30 * i_pred + 16 * k]);
		res31[k] = _mm256_loadu_si256((const __m256i*)&pred[31 * i_pred + 16 * k]);
		...
		//clip
		res00[k] = _mm256_min_epi16(res00[k], max_val);
		res00[k] = _mm256_max_epi16(res00[k], min_val);
		res01[k] = _mm256_min_epi16(res01[k], max_val);
		res01[k] = _mm256_max_epi16(res01[k], min_val);
		res02[k] = _mm256_min_epi16(res02[k], max_val);
		res02[k] = _mm256_max_epi16(res02[k], min_val);
		res03[k] = _mm256_min_epi16(res03[k], max_val);
		res03[k] = _mm256_max_epi16(res03[k], min_val);
		res04[k] = _mm256_min_epi16(res04[k], max_val);
		res04[k] = _mm256_max_epi16(res04[k], min_val);
		res05[k] = _mm256_min_epi16(res05[k], max_val);
		res05[k] = _mm256_max_epi16(res05[k], min_val);
		res06[k] = _mm256_min_epi16(res06[k], max_val);
		res06[k] = _mm256_max_epi16(res06[k], min_val);
		res07[k] = _mm256_min_epi16(res07[k], max_val);
		res07[k] = _mm256_max_epi16(res07[k], min_val);
		res08[k] = _mm256_min_epi16(res08[k], max_val);
		res08[k] = _mm256_max_epi16(res08[k], min_val);
		res09[k] = _mm256_min_epi16(res09[k], max_val);
		res09[k] = _mm256_max_epi16(res09[k], min_val);
		res10[k] = _mm256_min_epi16(res10[k], max_val);
		res10[k] = _mm256_max_epi16(res10[k], min_val);
		res11[k] = _mm256_min_epi16(res11[k], max_val);
		res11[k] = _mm256_max_epi16(res11[k], min_val);
		res12[k] = _mm256_min_epi16(res12[k], max_val);
		res12[k] = _mm256_max_epi16(res12[k], min_val);
		res13[k] = _mm256_min_epi16(res13[k], max_val);
		res13[k] = _mm256_max_epi16(res13[k], min_val);
		res14[k] = _mm256_min_epi16(res14[k], max_val);
		res14[k] = _mm256_max_epi16(res14[k], min_val);
		res15[k] = _mm256_min_epi16(res15[k], max_val);
		res15[k] = _mm256_max_epi16(res15[k], min_val);
		res16[k] = _mm256_min_epi16(res16[k], max_val);
		res16[k] = _mm256_max_epi16(res16[k], min_val);
		res17[k] = _mm256_min_epi16(res17[k], max_val);
		res17[k] = _mm256_max_epi16(res17[k], min_val);
		res18[k] = _mm256_min_epi16(res18[k], max_val);
		res18[k] = _mm256_max_epi16(res18[k], min_val);
		res19[k] = _mm256_min_epi16(res19[k], max_val);
		res19[k] = _mm256_max_epi16(res19[k], min_val);
		res20[k] = _mm256_min_epi16(res20[k], max_val);
		res20[k] = _mm256_max_epi16(res20[k], min_val);
		res21[k] = _mm256_min_epi16(res21[k], max_val);
		res21[k] = _mm256_max_epi16(res21[k], min_val);
		res22[k] = _mm256_min_epi16(res22[k], max_val);
		res22[k] = _mm256_max_epi16(res22[k], min_val);
		res23[k] = _mm256_min_epi16(res23[k], max_val);
		res23[k] = _mm256_max_epi16(res23[k], min_val);
		res24[k] = _mm256_min_epi16(res24[k], max_val);
		res24[k] = _mm256_max_epi16(res24[k], min_val);
		res25[k] = _mm256_min_epi16(res25[k], max_val);
		res25[k] = _mm256_max_epi16(res25[k], min_val);
		res26[k] = _mm256_min_epi16(res26[k], max_val);
		res26[k] = _mm256_max_epi16(res26[k], min_val);
		res27[k] = _mm256_min_epi16(res27[k], max_val);
		res27[k] = _mm256_max_epi16(res27[k], min_val);
		res28[k] = _mm256_min_epi16(res28[k], max_val);
		res28[k] = _mm256_max_epi16(res28[k], min_val);
		res29[k] = _mm256_min_epi16(res29[k], max_val);
		res29[k] = _mm256_max_epi16(res29[k], min_val);
		res30[k] = _mm256_min_epi16(res30[k], max_val);
		res30[k] = _mm256_max_epi16(res30[k], min_val);
		res31[k] = _mm256_min_epi16(res31[k], max_val);
		res31[k] = _mm256_max_epi16(res31[k], min_val);

		//store
		_mm256_storeu_si256((__m256i*)&dst[0 + 16 * k], res00[k]);			_mm256_storeu_si256((__m256i*)&dst[8 * i_dst + 16 * k], res01[k]);
		_mm256_storeu_si256((__m256i*)&dst[1 * i_dst + 16 * k], res02[k]);	_mm256_storeu_si256((__m256i*)&dst[9 * i_dst + 16 * k], res03[k]);
		_mm256_storeu_si256((__m256i*)&dst[2 * i_dst + 16 * k], res04[k]);	_mm256_storeu_si256((__m256i*)&dst[10 * i_dst + 16 * k], res05[k]);
		_mm256_storeu_si256((__m256i*)&dst[3 * i_dst + 16 * k], res06[k]);	_mm256_storeu_si256((__m256i*)&dst[11 * i_dst + 16 * k], res07[k]);
		_mm256_storeu_si256((__m256i*)&dst[4 * i_dst + 16 * k], res08[k]);	_mm256_storeu_si256((__m256i*)&dst[12 * i_dst + 16 * k], res09[k]);
		_mm256_storeu_si256((__m256i*)&dst[5 * i_dst + 16 * k], res10[k]);	_mm256_storeu_si256((__m256i*)&dst[13 * i_dst + 16 * k], res11[k]);
		_mm256_storeu_si256((__m256i*)&dst[6 * i_dst + 16 * k], res12[k]);	_mm256_storeu_si256((__m256i*)&dst[14 * i_dst + 16 * k], res13[k]);
		_mm256_storeu_si256((__m256i*)&dst[7 * i_dst + 16 * k], res14[k]);	_mm256_storeu_si256((__m256i*)&dst[15 * i_dst + 16 * k], res15[k]);

		_mm256_storeu_si256((__m256i*)&dst[16 * i_dst + 16 * k], res16[k]);	_mm256_storeu_si256((__m256i*)&dst[24 * i_dst + 16 * k], res17[k]);
		_mm256_storeu_si256((__m256i*)&dst[17 * i_dst + 16 * k], res18[k]);	_mm256_storeu_si256((__m256i*)&dst[25 * i_dst + 16 * k], res19[k]);
		_mm256_storeu_si256((__m256i*)&dst[18 * i_dst + 16 * k], res20[k]);	_mm256_storeu_si256((__m256i*)&dst[26 * i_dst + 16 * k], res21[k]);
		_mm256_storeu_si256((__m256i*)&dst[19 * i_dst + 16 * k], res22[k]);	_mm256_storeu_si256((__m256i*)&dst[27 * i_dst + 16 * k], res23[k]);
		_mm256_storeu_si256((__m256i*)&dst[20 * i_dst + 16 * k], res24[k]);	_mm256_storeu_si256((__m256i*)&dst[28 * i_dst + 16 * k], res25[k]);
		_mm256_storeu_si256((__m256i*)&dst[21 * i_dst + 16 * k], res26[k]);	_mm256_storeu_si256((__m256i*)&dst[29 * i_dst + 16 * k], res27[k]);
		_mm256_storeu_si256((__m256i*)&dst[22 * i_dst + 16 * k], res28[k]);	_mm256_storeu_si256((__m256i*)&dst[30 * i_dst + 16 * k], res29[k]);
		_mm256_storeu_si256((__m256i*)&dst[23 * i_dst + 16 * k], res30[k]);	_mm256_storeu_si256((__m256i*)&dst[31 * i_dst + 16 * k], res31[k]);
	}
}
...
#undef TRANSPOSE_16x16_16BIT
	...
	//	int i;
	{
		...
		{
			int offset = (k << 4);
			V00[k] = _mm256_lddqu_si256((const __m256i*)&pred[0 + offset]);
			V01[k] = _mm256_lddqu_si256((const __m256i*)&pred[1 * i_pred + offset]);
			V02[k] = _mm256_lddqu_si256((const __m256i*)&pred[2 * i_pred + offset]);
			V03[k] = _mm256_lddqu_si256((const __m256i*)&pred[3 * i_pred + offset]);
			V04[k] = _mm256_lddqu_si256((const __m256i*)&pred[4 * i_pred + offset]);
			V05[k] = _mm256_lddqu_si256((const __m256i*)&pred[5 * i_pred + offset]);
			V06[k] = _mm256_lddqu_si256((const __m256i*)&pred[6 * i_pred + offset]);
			V07[k] = _mm256_lddqu_si256((const __m256i*)&pred[7 * i_pred + offset]);
			V08[k] = _mm256_lddqu_si256((const __m256i*)&pred[8 * i_pred + offset]);
			V09[k] = _mm256_lddqu_si256((const __m256i*)&pred[9 * i_pred + offset]);

			V10[k] = _mm256_lddqu_si256((const __m256i*)&pred[10 * i_pred + offset]);
			V11[k] = _mm256_lddqu_si256((const __m256i*)&pred[11 * i_pred + offset]);
			V12[k] = _mm256_lddqu_si256((const __m256i*)&pred[12 * i_pred + offset]);
			V13[k] = _mm256_lddqu_si256((const __m256i*)&pred[13 * i_pred + offset]);
			V14[k] = _mm256_lddqu_si256((const __m256i*)&pred[14 * i_pred + offset]);
			V15[k] = _mm256_lddqu_si256((const __m256i*)&pred[15 * i_pred + offset]);
			V16[k] = _mm256_lddqu_si256((const __m256i*)&pred[16 * i_pred + offset]);
			V17[k] = _mm256_lddqu_si256((const __m256i*)&pred[17 * i_pred + offset]);
			V18[k] = _mm256_lddqu_si256((const __m256i*)&pred[18 * i_pred + offset]);
			V19[k] = _mm256_lddqu_si256((const __m256i*)&pred[19 * i_pred + offset]);


			V20[k] = _mm256_lddqu_si256((const __m256i*)&pred[20 * i_pred + offset]);
			V21[k] = _mm256_lddqu_si256((const __m256i*)&pred[21 * i_pred + offset]);
			V22[k] = _mm256_lddqu_si256((const __m256i*)&pred[22 * i_pred + offset]);
			V23[k] = _mm256_lddqu_si256((const __m256i*)&pred[23 * i_pred + offset]);
			V24[k] = _mm256_lddqu_si256((const __m256i*)&pred[24 * i_pred + offset]);
			V25[k] = _mm256_lddqu_si256((const __m256i*)&pred[25 * i_pred + offset]);
			V26[k] = _mm256_lddqu_si256((const __m256i*)&pred[26 * i_pred + offset]);
			V27[k] = _mm256_lddqu_si256((const __m256i*)&pred[27 * i_pred + offset]);
			V28[k] = _mm256_lddqu_si256((const __m256i*)&pred[28 * i_pred + offset]);
			V29[k] = _mm256_lddqu_si256((const __m256i*)&pred[29 * i_pred + offset]);

			V30[k] = _mm256_lddqu_si256((const __m256i*)&pred[30 * i_pred + offset]);
			V31[k] = _mm256_lddqu_si256((const __m256i*)&pred[31 * i_pred + offset]);
			V32[k] = _mm256_lddqu_si256((const __m256i*)&pred[32 * i_pred + offset]);
			V33[k] = _mm256_lddqu_si256((const __m256i*)&pred[33 * i_pred + offset]);
			V34[k] = _mm256_lddqu_si256((const __m256i*)&pred[34 * i_pred + offset]);
			V35[k] = _mm256_lddqu_si256((const __m256i*)&pred[35 * i_pred + offset]);
			V36[k] = _mm256_lddqu_si256((const __m256i*)&pred[36 * i_pred + offset]);
			V37[k] = _mm256_lddqu_si256((const __m256i*)&pred[37 * i_pred + offset]);
			V38[k] = _mm256_lddqu_si256((const __m256i*)&pred[38 * i_pred + offset]);
			V39[k] = _mm256_lddqu_si256((const __m256i*)&pred[39 * i_pred + offset]);

			V40[k] = _mm256_lddqu_si256((const __m256i*)&pred[40 * i_pred + offset]);
			V41[k] = _mm256_lddqu_si256((const __m256i*)&pred[41 * i_pred + offset]);
			V42[k] = _mm256_lddqu_si256((const __m256i*)&pred[42 * i_pred + offset]);
			V43[k] = _mm256_lddqu_si256((const __m256i*)&pred[43 * i_pred + offset]);
			V44[k] = _mm256_lddqu_si256((const __m256i*)&pred[44 * i_pred + offset]);
			V45[k] = _mm256_lddqu_si256((const __m256i*)&pred[45 * i_pred + offset]);
			V46[k] = _mm256_lddqu_si256((const __m256i*)&pred[46 * i_pred + offset]);
			V47[k] = _mm256_lddqu_si256((const __m256i*)&pred[47 * i_pred + offset]);
			V48[k] = _mm256_lddqu_si256((const __m256i*)&pred[48 * i_pred + offset]);
			V49[k] = _mm256_lddqu_si256((const __m256i*)&pred[49 * i_pred + offset]);

			V50[k] = _mm256_lddqu_si256((const __m256i*)&pred[50 * i_pred + offset]);
			V51[k] = _mm256_lddqu_si256((const __m256i*)&pred[51 * i_pred + offset]);
			V52[k] = _mm256_lddqu_si256((const __m256i*)&pred[52 * i_pred + offset]);
			V53[k] = _mm256_lddqu_si256((const __m256i*)&pred[53 * i_pred + offset]);
			V54[k] = _mm256_lddqu_si256((const __m256i*)&pred[54 * i_pred + offset]);
			V55[k] = _mm256_lddqu_si256((const __m256i*)&pred[55 * i_pred + offset]);
			V56[k] = _mm256_lddqu_si256((const __m256i*)&pred[56 * i_pred + offset]);
			V57[k] = _mm256_lddqu_si256((const __m256i*)&pred[57 * i_pred + offset]);
			V58[k] = _mm256_lddqu_si256((const __m256i*)&pred[58 * i_pred + offset]);
			V59[k] = _mm256_lddqu_si256((const __m256i*)&pred[59 * i_pred + offset]);

			V60[k] = _mm256_lddqu_si256((const __m256i*)&pred[60 * i_pred + offset]);
			V61[k] = _mm256_lddqu_si256((const __m256i*)&pred[61 * i_pred + offset]);
			V62[k] = _mm256_lddqu_si256((const __m256i*)&pred[62 * i_pred + offset]);
			V63[k] = _mm256_lddqu_si256((const __m256i*)&pred[63 * i_pred + offset]);
		}
		...
		{
			int offset = (k << 4);
			max_val = _mm256_set1_epi16(vmax2);
			min_val = zero;
			...
			//CLIP
			T00[k] = _mm256_min_epi16(T00[k], max_val);
			T00[k] = _mm256_max_epi16(T00[k], min_val);
			T01[k] = _mm256_min_epi16(T01[k], max_val);
			T01[k] = _mm256_max_epi16(T01[k], min_val);
			T02[k] = _mm256_min_epi16(T02[k], max_val);
			T02[k] = _mm256_max_epi16(T02[k], min_val);
			T03[k] = _mm256_min_epi16(T03[k], max_val);
			T03[k] = _mm256_max_epi16(T03[k], min_val);
			T04[k] = _mm256_min_epi16(T04[k], max_val);
			T04[k] = _mm256_max_epi16(T04[k], min_val);
			T05[k] = _mm256_min_epi16(T05[k], max_val);
			T05[k] = _mm256_max_epi16(T05[k], min_val);
			T06[k] = _mm256_min_epi16(T06[k], max_val);
			T06[k] = _mm256_max_epi16(T06[k], min_val);
			T07[k] = _mm256_min_epi16(T07[k], max_val);
			T07[k] = _mm256_max_epi16(T07[k], min_val);
			T08[k] = _mm256_min_epi16(T08[k], max_val);
			T08[k] = _mm256_max_epi16(T08[k], min_val);
			T09[k] = _mm256_min_epi16(T09[k], max_val);
			T09[k] = _mm256_max_epi16(T09[k], min_val);
			T10[k] = _mm256_min_epi16(T10[k], max_val);
			T10[k] = _mm256_max_epi16(T10[k], min_val);
			T11[k] = _mm256_min_epi16(T11[k], max_val);
			T11[k] = _mm256_max_epi16(T11[k], min_val);
			T12[k] = _mm256_min_epi16(T12[k], max_val);
			T12[k] = _mm256_max_epi16(T12[k], min_val);
			T13[k] = _mm256_min_epi16(T13[k], max_val);
			T13[k] = _mm256_max_epi16(T13[k], min_val);
			T14[k] = _mm256_min_epi16(T14[k], max_val);
			T14[k] = _mm256_max_epi16(T14[k], min_val);
			T15[k] = _mm256_min_epi16(T15[k], max_val);
			T15[k] = _mm256_max_epi16(T15[k], min_val);
			T16[k] = _mm256_min_epi16(T16[k], max_val);
			T16[k] = _mm256_max_epi16(T16[k], min_val);
			T17[k] = _mm256_min_epi16(T17[k], max_val);
			T17[k] = _mm256_max_epi16(T17[k], min_val);
			T18[k] = _mm256_min_epi16(T18[k], max_val);
			T18[k] = _mm256_max_epi16(T18[k], min_val);
			T19[k] = _mm256_min_epi16(T19[k], max_val);
			T19[k] = _mm256_max_epi16(T19[k], min_val);
			T20[k] = _mm256_min_epi16(T20[k], max_val);
			T20[k] = _mm256_max_epi16(T20[k], min_val);
			T21[k] = _mm256_min_epi16(T21[k], max_val);
			T21[k] = _mm256_max_epi16(T21[k], min_val);
			T22[k] = _mm256_min_epi16(T22[k], max_val);
			T22[k] = _mm256_max_epi16(T22[k], min_val);
			T23[k] = _mm256_min_epi16(T23[k], max_val);
			T23[k] = _mm256_max_epi16(T23[k], min_val);
			T24[k] = _mm256_min_epi16(T24[k], max_val);
			T24[k] = _mm256_max_epi16(T24[k], min_val);
			T25[k] = _mm256_min_epi16(T25[k], max_val);
			T25[k] = _mm256_max_epi16(T25[k], min_val);
			T26[k] = _mm256_min_epi16(T26[k], max_val);
			T26[k] = _mm256_max_epi16(T26[k], min_val);
			T27[k] = _mm256_min_epi16(T27[k], max_val);
			T27[k] = _mm256_max_epi16(T27[k], min_val);
			T28[k] = _mm256_min_epi16(T28[k], max_val);
			T28[k] = _mm256_max_epi16(T28[k], min_val);
			T29[k] = _mm256_min_epi16(T29[k], max_val);
			T29[k] = _mm256_max_epi16(T29[k], min_val);
			T30[k] = _mm256_min_epi16(T30[k], max_val);
			T30[k] = _mm256_max_epi16(T30[k], min_val);
			T31[k] = _mm256_min_epi16(T31[k], max_val);
			T31[k] = _mm256_max_epi16(T31[k], min_val);
			T32[k] = _mm256_min_epi16(T32[k], max_val);
			T32[k] = _mm256_max_epi16(T32[k], min_val);
			T33[k] = _mm256_min_epi16(T33[k], max_val);
			T33[k] = _mm256_max_epi16(T33[k], min_val);
			T34[k] = _mm256_min_epi16(T34[k], max_val);
			T34[k] = _mm256_max_epi16(T34[k], min_val);
			T35[k] = _mm256_min_epi16(T35[k], max_val);
			T35[k] = _mm256_max_epi16(T35[k], min_val);
			T36[k] = _mm256_min_epi16(T36[k], max_val);
			T36[k] = _mm256_max_epi16(T36[k], min_val);
			T37[k] = _mm256_min_epi16(T37[k], max_val);
			T37[k] = _mm256_max_epi16(T37[k], min_val);
			T38[k] = _mm256_min_epi16(T38[k], max_val);
			T38[k] = _mm256_max_epi16(T38[k], min_val);
			T39[k] = _mm256_min_epi16(T39[k], max_val);
			T39[k] = _mm256_max_epi16(T39[k], min_val);
			T40[k] = _mm256_min_epi16(T40[k], max_val);
			T40[k] = _mm256_max_epi16(T40[k], min_val);
			T41[k] = _mm256_min_epi16(T41[k], max_val);
			T41[k] = _mm256_max_epi16(T41[k], min_val);
			T42[k] = _mm256_min_epi16(T42[k], max_val);
			T42[k] = _mm256_max_epi16(T42[k], min_val);
			T43[k] = _mm256_min_epi16(T43[k], max_val);
			T43[k] = _mm256_max_epi16(T43[k], min_val);
			T44[k] = _mm256_min_epi16(T44[k], max_val);
			T44[k] = _mm256_max_epi16(T44[k], min_val);
			T45[k] = _mm256_min_epi16(T45[k], max_val);
			T45[k] = _mm256_max_epi16(T45[k], min_val);
			T46[k] = _mm256_min_epi16(T46[k], max_val);
			T46[k] = _mm256_max_epi16(T46[k], min_val);
			T47[k] = _mm256_min_epi16(T47[k], max_val);
			T47[k] = _mm256_max_epi16(T47[k], min_val);
			T48[k] = _mm256_min_epi16(T48[k], max_val);
			T48[k] = _mm256_max_epi16(T48[k], min_val);
			T49[k] = _mm256_min_epi16(T49[k], max_val);
			T49[k] = _mm256_max_epi16(T49[k], min_val);
			T50[k] = _mm256_min_epi16(T50[k], max_val);
			T50[k] = _mm256_max_epi16(T50[k], min_val);
			T51[k] = _mm256_min_epi16(T51[k], max_val);
			T51[k] = _mm256_max_epi16(T51[k], min_val);
			T52[k] = _mm256_min_epi16(T52[k], max_val);
			T52[k] = _mm256_max_epi16(T52[k], min_val);
			T53[k] = _mm256_min_epi16(T53[k], max_val);
			T53[k] = _mm256_max_epi16(T53[k], min_val);
			T54[k] = _mm256_min_epi16(T54[k], max_val);
			T54[k] = _mm256_max_epi16(T54[k], min_val);
			T55[k] = _mm256_min_epi16(T55[k], max_val);
			T55[k] = _mm256_max_epi16(T55[k], min_val);
			T56[k] = _mm256_min_epi16(T56[k], max_val);
			T56[k] = _mm256_max_epi16(T56[k], min_val);
			T57[k] = _mm256_min_epi16(T57[k], max_val);
			T57[k] = _mm256_max_epi16(T57[k], min_val);
			T58[k] = _mm256_min_epi16(T58[k], max_val);
			T58[k] = _mm256_max_epi16(T58[k], min_val);
			T59[k] = _mm256_min_epi16(T59[k], max_val);
			T59[k] = _mm256_max_epi16(T59[k], min_val);
			T60[k] = _mm256_min_epi16(T60[k], max_val);
			T60[k] = _mm256_max_epi16(T60[k], min_val);
			T61[k] = _mm256_min_epi16(T61[k], max_val);
			T61[k] = _mm256_max_epi16(T61[k], min_val);
			T62[k] = _mm256_min_epi16(T62[k], max_val);
			T62[k] = _mm256_max_epi16(T62[k], min_val);
			T63[k] = _mm256_min_epi16(T63[k], max_val);
			T63[k] = _mm256_max_epi16(T63[k], min_val);

			//STORE
			_mm256_storeu_si256((__m256i*)&dst[0 + offset], T00[k]);
			_mm256_storeu_si256((__m256i*)&dst[1 * i_dst + offset], T01[k]);
			_mm256_storeu_si256((__m256i*)&dst[2 * i_dst + offset], T02[k]);
			_mm256_storeu_si256((__m256i*)&dst[3 * i_dst + offset], T03[k]);
			_mm256_storeu_si256((__m256i*)&dst[4 * i_dst + offset], T04[k]);
			_mm256_storeu_si256((__m256i*)&dst[5 * i_dst + offset], T05[k]);
			_mm256_storeu_si256((__m256i*)&dst[6 * i_dst + offset], T06[k]);
			_mm256_storeu_si256((__m256i*)&dst[7 * i_dst + offset], T07[k]);
			_mm256_storeu_si256((__m256i*)&dst[8 * i_dst + offset], T08[k]);
			_mm256_storeu_si256((__m256i*)&dst[9 * i_dst + offset], T09[k]);

			_mm256_storeu_si256((__m256i*)&dst[10 * i_dst + offset], T10[k]);
			_mm256_storeu_si256((__m256i*)&dst[11 * i_dst + offset], T11[k]);
			_mm256_storeu_si256((__m256i*)&dst[12 * i_dst + offset], T12[k]);
			_mm256_storeu_si256((__m256i*)&dst[13 * i_dst + offset], T13[k]);
			_mm256_storeu_si256((__m256i*)&dst[14 * i_dst + offset], T14[k]);
			_mm256_storeu_si256((__m256i*)&dst[15 * i_dst + offset], T15[k]);
			_mm256_storeu_si256((__m256i*)&dst[16 * i_dst + offset], T16[k]);
			_mm256_storeu_si256((__m256i*)&dst[17 * i_dst + offset], T17[k]);
			_mm256_storeu_si256((__m256i*)&dst[18 * i_dst + offset], T18[k]);
			_mm256_storeu_si256((__m256i*)&dst[19 * i_dst + offset], T19[k]);

			_mm256_storeu_si256((__m256i*)&dst[20 * i_dst + offset], T20[k]);
			_mm256_storeu_si256((__m256i*)&dst[21 * i_dst + offset], T21[k]);
			_mm256_storeu_si256((__m256i*)&dst[22 * i_dst + offset], T22[k]);
			_mm256_storeu_si256((__m256i*)&dst[23 * i_dst + offset], T23[k]);
			_mm256_storeu_si256((__m256i*)&dst[24 * i_dst + offset], T24[k]);
			_mm256_storeu_si256((__m256i*)&dst[25 * i_dst + offset], T25[k]);
			_mm256_storeu_si256((__m256i*)&dst[26 * i_dst + offset], T26[k]);
			_mm256_storeu_si256((__m256i*)&dst[27 * i_dst + offset], T27[k]);
			_mm256_storeu_si256((__m256i*)&dst[28 * i_dst + offset], T28[k]);
			_mm256_storeu_si256((__m256i*)&dst[29 * i_dst + offset], T29[k]);

			_mm256_storeu_si256((__m256i*)&dst[30 * i_dst + offset], T30[k]);
			_mm256_storeu_si256((__m256i*)&dst[31 * i_dst + offset], T31[k]);
			_mm256_storeu_si256((__m256i*)&dst[32 * i_dst + offset], T32[k]);
			_mm256_storeu_si256((__m256i*)&dst[33 * i_dst + offset], T33[k]);
			_mm256_storeu_si256((__m256i*)&dst[34 * i_dst + offset], T34[k]);
			_mm256_storeu_si256((__m256i*)&dst[35 * i_dst + offset], T35[k]);
			_mm256_storeu_si256((__m256i*)&dst[36 * i_dst + offset], T36[k]);
			_mm256_storeu_si256((__m256i*)&dst[37 * i_dst + offset], T37[k]);
			_mm256_storeu_si256((__m256i*)&dst[38 * i_dst + offset], T38[k]);
			_mm256_storeu_si256((__m256i*)&dst[39 * i_dst + offset], T39[k]);

			_mm256_storeu_si256((__m256i*)&dst[40 * i_dst + offset], T40[k]);
			_mm256_storeu_si256((__m256i*)&dst[41 * i_dst + offset], T41[k]);
			_mm256_storeu_si256((__m256i*)&dst[42 * i_dst + offset], T42[k]);
			_mm256_storeu_si256((__m256i*)&dst[43 * i_dst + offset], T43[k]);
			_mm256_storeu_si256((__m256i*)&dst[44 * i_dst + offset], T44[k]);
			_mm256_storeu_si256((__m256i*)&dst[45 * i_dst + offset], T45[k]);
			_mm256_storeu_si256((__m256i*)&dst[46 * i_dst + offset], T46[k]);
			_mm256_storeu_si256((__m256i*)&dst[47 * i_dst + offset], T47[k]);
			_mm256_storeu_si256((__m256i*)&dst[48 * i_dst + offset], T48[k]);
			_mm256_storeu_si256((__m256i*)&dst[49 * i_dst + offset], T49[k]);

			_mm256_storeu_si256((__m256i*)&dst[50 * i_dst + offset], T50[k]);
			_mm256_storeu_si256((__m256i*)&dst[51 * i_dst + offset], T51[k]);
			_mm256_storeu_si256((__m256i*)&dst[52 * i_dst + offset], T52[k]);
			_mm256_storeu_si256((__m256i*)&dst[53 * i_dst + offset], T53[k]);
			_mm256_storeu_si256((__m256i*)&dst[54 * i_dst + offset], T54[k]);
			_mm256_storeu_si256((__m256i*)&dst[55 * i_dst + offset], T55[k]);
			_mm256_storeu_si256((__m256i*)&dst[56 * i_dst + offset], T56[k]);
			_mm256_storeu_si256((__m256i*)&dst[57 * i_dst + offset], T57[k]);
			_mm256_storeu_si256((__m256i*)&dst[58 * i_dst + offset], T58[k]);
			_mm256_storeu_si256((__m256i*)&dst[59 * i_dst + offset], T59[k]);

			_mm256_storeu_si256((__m256i*)&dst[60 * i_dst + offset], T60[k]);
			_mm256_storeu_si256((__m256i*)&dst[61 * i_dst + offset], T61[k]);
			_mm256_storeu_si256((__m256i*)&dst[62 * i_dst + offset], T62[k]);
			_mm256_storeu_si256((__m256i*)&dst[63 * i_dst + offset], T63[k]);
