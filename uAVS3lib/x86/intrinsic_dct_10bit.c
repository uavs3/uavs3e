...
void sub_trans_4x4_sec_10bit_sse128(pel_t *org, int i_org, pel_t *pred, int i_pred, coef_t *dst, int shift){
    ...
    O00 = _mm_loadl_epi64((__m128i*)&org[0 * i_org]); //low 64 bits is valid!
	...
	/*---hor transform---*/
	// shift = shift + 1; rnd_factor = 
    shift += 1;
    factor = _mm_set1_epi32(shift == 0 ? 0 : 1 << (shift - 1));
	...
	//shift operation
    tmpRes0 = _mm_packs_epi32(_mm_srai_epi32(tmpSum3, shift), zero);			//!only low 64bits (4xSHORT) are valid!
	...
	tmpSum3 = _mm_add_epi32(tmpSum2, factor);
	//shift operation
    tmpRes1 = _mm_packs_epi32(_mm_srai_epi32(tmpSum3, shift), zero);			//!only low 64bits (4xSHORT) are valid!
	...
	tmpSum3 = _mm_add_epi32(tmpSum2, factor);
	//shift operation
    tmpRes2 = _mm_packs_epi32(_mm_srai_epi32(tmpSum3, shift), zero);			//!only low 64bits (4xSHORT) are valid!
	...
	tmpSum3 = _mm_add_epi32(tmpSum2, factor);
	//shift operation
    tmpRes3 = _mm_packs_epi32(_mm_srai_epi32(tmpSum3, shift), zero);				//!only low 64bits (4xSHORT) are valid!
...
void sub_trans_4x4_10bit_sse128(pel_t *org, int i_org, pel_t *pred, int i_pred, coef_t *dst, int shift){
    ...
    const __m128i k_ROUNDING1 = _mm_set1_epi32(shift ? (1 << (shift - 1)) : 0);
	...
	res0 = _mm_packs_epi32(w0, w1);	//out[0] out[1]
	res1 = _mm_packs_epi32(w2, w3);	//out[2] out[3]
...
void sub_trans_8x8_10bit_sse128(pel_t *org, int i_org, pel_t *pred, int i_pred, coef_t *dst, int shift){
    int shift1 = shift;
    int shift2 = 8;
    const __m128i k_ROUNDING1 = _mm_set1_epi32(1 << (shift1 - 1));
...
{
    // Const
    __m128i c_2 = _mm_set1_epi32(1 << (shift - 1));	//shift1 = shift
    ...
    {
        //input data
        O00A = _mm_loadu_si128((__m128i*)&org[0 * i_org]);
        O00B = _mm_loadu_si128((__m128i*)&org[0 * i_org + 8]);
        O01A = _mm_loadu_si128((__m128i*)&org[1 * i_org]);
        O01B = _mm_loadu_si128((__m128i*)&org[1 * i_org + 8]);
        O02A = _mm_loadu_si128((__m128i*)&org[2 * i_org]);
        O02B = _mm_loadu_si128((__m128i*)&org[2 * i_org + 8]);
        O03A = _mm_loadu_si128((__m128i*)&org[3 * i_org]);
        O03B = _mm_loadu_si128((__m128i*)&org[3 * i_org + 8]);
        O04A = _mm_loadu_si128((__m128i*)&org[4 * i_org]);
        O04B = _mm_loadu_si128((__m128i*)&org[4 * i_org + 8]);
        O05A = _mm_loadu_si128((__m128i*)&org[5 * i_org]);
        O05B = _mm_loadu_si128((__m128i*)&org[5 * i_org + 8]);
        O06A = _mm_loadu_si128((__m128i*)&org[6 * i_org]);
        O06B = _mm_loadu_si128((__m128i*)&org[6 * i_org + 8]);
        O07A = _mm_loadu_si128((__m128i*)&org[7 * i_org]);
        O07B = _mm_loadu_si128((__m128i*)&org[7 * i_org + 8]);

        org += i_org << 3;

        P00A = _mm_loadu_si128((__m128i*)&pred[0 * i_pred]);
        P00B = _mm_loadu_si128((__m128i*)&pred[0 * i_pred + 8]);
        P01A = _mm_loadu_si128((__m128i*)&pred[1 * i_pred]);
        P01B = _mm_loadu_si128((__m128i*)&pred[1 * i_pred + 8]);
        P02A = _mm_loadu_si128((__m128i*)&pred[2 * i_pred]);
        P02B = _mm_loadu_si128((__m128i*)&pred[2 * i_pred + 8]);
        P03A = _mm_loadu_si128((__m128i*)&pred[3 * i_pred]);
        P03B = _mm_loadu_si128((__m128i*)&pred[3 * i_pred + 8]);
        P04A = _mm_loadu_si128((__m128i*)&pred[4 * i_pred]);
        P04B = _mm_loadu_si128((__m128i*)&pred[4 * i_pred + 8]);
        P05A = _mm_loadu_si128((__m128i*)&pred[5 * i_pred]);
        P05B = _mm_loadu_si128((__m128i*)&pred[5 * i_pred + 8]);
        P06A = _mm_loadu_si128((__m128i*)&pred[6 * i_pred]);
        P06B = _mm_loadu_si128((__m128i*)&pred[6 * i_pred + 8]);
        P07A = _mm_loadu_si128((__m128i*)&pred[7 * i_pred]);
        P07B = _mm_loadu_si128((__m128i*)&pred[7 * i_pred + 8]);
...
void sub_trans_16x16_10bit_sse256(pel_t *org, int i_org, pel_t *pred, int i_pred, coef_t *dst, int shift)
{
	...
	int shift1 = shift;
	int shift2 = 9;
	__m256i c_2 = _mm256_set1_epi32(1 << (shift1 - 1));
	...
	// DCT1
	//input data
	O00A = _mm256_loadu_si256((const __m256i*)&org[0]);
	O00B = _mm256_loadu_si256((const __m256i*)&org[8 * i_org]);
	O01A = _mm256_loadu_si256((const __m256i*)&org[1 * i_org]);
	O01B = _mm256_loadu_si256((const __m256i*)&org[9 * i_org]);
	O02A = _mm256_loadu_si256((const __m256i*)&org[2 * i_org]);
	O02B = _mm256_loadu_si256((const __m256i*)&org[10 * i_org]);
	O03A = _mm256_loadu_si256((const __m256i*)&org[3 * i_org]);
	O03B = _mm256_loadu_si256((const __m256i*)&org[11 * i_org]);
	O04A = _mm256_loadu_si256((const __m256i*)&org[4 * i_org]);
	O04B = _mm256_loadu_si256((const __m256i*)&org[12 * i_org]);
	O05A = _mm256_loadu_si256((const __m256i*)&org[5 * i_org]);
	O05B = _mm256_loadu_si256((const __m256i*)&org[13 * i_org]);
	O06A = _mm256_loadu_si256((const __m256i*)&org[6 * i_org]);
	O06B = _mm256_loadu_si256((const __m256i*)&org[14 * i_org]);
	O07A = _mm256_loadu_si256((const __m256i*)&org[7 * i_org]);
	O07B = _mm256_loadu_si256((const __m256i*)&org[15 * i_org]);

	P00A = _mm256_loadu_si256((const __m256i*)&pred[0]);
	P00B = _mm256_loadu_si256((const __m256i*)&pred[8 * i_pred]);
	P01A = _mm256_loadu_si256((const __m256i*)&pred[1 * i_pred]);
	P01B = _mm256_loadu_si256((const __m256i*)&pred[9 * i_pred]);
	P02A = _mm256_loadu_si256((const __m256i*)&pred[2 * i_pred]);
	P02B = _mm256_loadu_si256((const __m256i*)&pred[10 * i_pred]);
	P03A = _mm256_loadu_si256((const __m256i*)&pred[3 * i_pred]);
	P03B = _mm256_loadu_si256((const __m256i*)&pred[11 * i_pred]);
	P04A = _mm256_loadu_si256((const __m256i*)&pred[4 * i_pred]);
	P04B = _mm256_loadu_si256((const __m256i*)&pred[12 * i_pred]);
	P05A = _mm256_loadu_si256((const __m256i*)&pred[5 * i_pred]);
	P05B = _mm256_loadu_si256((const __m256i*)&pred[13 * i_pred]);
	P06A = _mm256_loadu_si256((const __m256i*)&pred[6 * i_pred]);
	P06B = _mm256_loadu_si256((const __m256i*)&pred[14 * i_pred]);
	P07A = _mm256_loadu_si256((const __m256i*)&pred[7 * i_pred]);
	P07B = _mm256_loadu_si256((const __m256i*)&pred[15 * i_pred]);

	im[0] = _mm256_sub_epi16(O00A, P00A);
	im[8] = _mm256_sub_epi16(O00B, P00B);
	im[1] = _mm256_sub_epi16(O01A, P01A);
	im[9] = _mm256_sub_epi16(O01B, P01B);
	im[2] = _mm256_sub_epi16(O02A, P02A);
	im[10] = _mm256_sub_epi16(O02B, P02B);
	im[3] = _mm256_sub_epi16(O03A, P03A);
	im[11] = _mm256_sub_epi16(O03B, P03B);
	im[4] = _mm256_sub_epi16(O04A, P04A);
	im[12] = _mm256_sub_epi16(O04B, P04B);
	im[5] = _mm256_sub_epi16(O05A, P05A);
	im[13] = _mm256_sub_epi16(O05B, P05B);
	im[6] = _mm256_sub_epi16(O06A, P06A);
	im[14] = _mm256_sub_epi16(O06B, P06B);
	im[7] = _mm256_sub_epi16(O07A, P07A);
	im[15] = _mm256_sub_epi16(O07B, P07B);
...
void sub_trans_32x32_10bit_sse128(pel_t *org, int i_org, pel_t *pred, int i_pred, coef_t *dst, int shift){
    // Const
    __m128i c_4 = _mm_set1_epi32(1 << (shift - 1));
    ...
    for (i = 0; i < 32 / 8; i++)
    {
        ...
        O00A = _mm_loadu_si128((__m128i*)&org[0 * i_org + 16]);
        O00B = _mm_loadu_si128((__m128i*)&org[0 * i_org + 24]);
        O01A = _mm_loadu_si128((__m128i*)&org[1 * i_org + 16]);
        O01B = _mm_loadu_si128((__m128i*)&org[1 * i_org + 24]);
        O02A = _mm_loadu_si128((__m128i*)&org[2 * i_org + 16]);
        O02B = _mm_loadu_si128((__m128i*)&org[2 * i_org + 24]);
        O03A = _mm_loadu_si128((__m128i*)&org[3 * i_org + 16]);
        O03B = _mm_loadu_si128((__m128i*)&org[3 * i_org + 24]);
        O04A = _mm_loadu_si128((__m128i*)&org[4 * i_org + 16]);
        O04B = _mm_loadu_si128((__m128i*)&org[4 * i_org + 24]);
        O05A = _mm_loadu_si128((__m128i*)&org[5 * i_org + 16]);
        O05B = _mm_loadu_si128((__m128i*)&org[5 * i_org + 24]);
        O06A = _mm_loadu_si128((__m128i*)&org[6 * i_org + 16]);
        O06B = _mm_loadu_si128((__m128i*)&org[6 * i_org + 24]);
        O07A = _mm_loadu_si128((__m128i*)&org[7 * i_org + 16]);
        O07B = _mm_loadu_si128((__m128i*)&org[7 * i_org + 24]);

        P00A = _mm_loadu_si128((__m128i*)&pred[0 * i_pred + 16]);
        P00B = _mm_loadu_si128((__m128i*)&pred[0 * i_pred + 24]);
        P01A = _mm_loadu_si128((__m128i*)&pred[1 * i_pred + 16]);
        P01B = _mm_loadu_si128((__m128i*)&pred[1 * i_pred + 24]);
        P02A = _mm_loadu_si128((__m128i*)&pred[2 * i_pred + 16]);
        P02B = _mm_loadu_si128((__m128i*)&pred[2 * i_pred + 24]);
        P03A = _mm_loadu_si128((__m128i*)&pred[3 * i_pred + 16]);
        P03B = _mm_loadu_si128((__m128i*)&pred[3 * i_pred + 24]);
        P04A = _mm_loadu_si128((__m128i*)&pred[4 * i_pred + 16]);
        P04B = _mm_loadu_si128((__m128i*)&pred[4 * i_pred + 24]);
        P05A = _mm_loadu_si128((__m128i*)&pred[5 * i_pred + 16]);
        P05B = _mm_loadu_si128((__m128i*)&pred[5 * i_pred + 24]);
        P06A = _mm_loadu_si128((__m128i*)&pred[6 * i_pred + 16]);
        P06B = _mm_loadu_si128((__m128i*)&pred[6 * i_pred + 24]);
        P07A = _mm_loadu_si128((__m128i*)&pred[7 * i_pred + 16]);
        P07B = _mm_loadu_si128((__m128i*)&pred[7 * i_pred + 24]);
...
void sub_trans_32x32_10bit_sse256(pel_t *org, int i_org, pel_t *pred, int i_pred, coef_t *dst, int shift){
	...
	int shift1 = shift;
	int shift2 = 10;
	__m256i c_4 = _mm256_set1_epi32(1 << (shift - 1));
	...
	__m256i P00, P01, P02, P03, P04, P05, P06, P07, P08, P09, P10, P11, P12, P13, P14, P15;
	__m256i P00A, P01A, P02A, P03A, P04A, P05A, P06A, P07A, P08A, P09A, P10A, P11A, P12A, P13A, P14A, P15A;
	__m256i P00B, P01B, P02B, P03B, P04B, P05B, P06B, P07B, P08B, P09B, P10B, P11B, P12B, P13B, P14B, P15B;
	__m256i tmpZero = _mm256_setzero_si256();
	...
	{
		//load data
		O00A = _mm256_loadu_si256((__m256i*)&org[0 * i_org]);
		O01A = _mm256_loadu_si256((__m256i*)&org[1 * i_org]);
		O02A = _mm256_loadu_si256((__m256i*)&org[2 * i_org]);
		O03A = _mm256_loadu_si256((__m256i*)&org[3 * i_org]);
		O04A = _mm256_loadu_si256((__m256i*)&org[4 * i_org]);
		O05A = _mm256_loadu_si256((__m256i*)&org[5 * i_org]);
		O06A = _mm256_loadu_si256((__m256i*)&org[6 * i_org]);
		O07A = _mm256_loadu_si256((__m256i*)&org[7 * i_org]);
		O08A = _mm256_loadu_si256((__m256i*)&org[8 * i_org]);
		O09A = _mm256_loadu_si256((__m256i*)&org[9 * i_org]);
		O10A = _mm256_loadu_si256((__m256i*)&org[10 * i_org]);
		O11A = _mm256_loadu_si256((__m256i*)&org[11 * i_org]);
		O12A = _mm256_loadu_si256((__m256i*)&org[12 * i_org]);
		O13A = _mm256_loadu_si256((__m256i*)&org[13 * i_org]);
		O14A = _mm256_loadu_si256((__m256i*)&org[14 * i_org]);
		O15A = _mm256_loadu_si256((__m256i*)&org[15 * i_org]);
		P00A = _mm256_loadu_si256((__m256i*)&pred[0 * i_pred]);
		P01A = _mm256_loadu_si256((__m256i*)&pred[1 * i_pred]);
		P02A = _mm256_loadu_si256((__m256i*)&pred[2 * i_pred]);
		P03A = _mm256_loadu_si256((__m256i*)&pred[3 * i_pred]);
		P04A = _mm256_loadu_si256((__m256i*)&pred[4 * i_pred]);
		P05A = _mm256_loadu_si256((__m256i*)&pred[5 * i_pred]);
		P06A = _mm256_loadu_si256((__m256i*)&pred[6 * i_pred]);
		P07A = _mm256_loadu_si256((__m256i*)&pred[7 * i_pred]);
		P08A = _mm256_loadu_si256((__m256i*)&pred[8 * i_pred]);
		P09A = _mm256_loadu_si256((__m256i*)&pred[9 * i_pred]);
		P10A = _mm256_loadu_si256((__m256i*)&pred[10 * i_pred]);
		P11A = _mm256_loadu_si256((__m256i*)&pred[11 * i_pred]);
		P12A = _mm256_loadu_si256((__m256i*)&pred[12 * i_pred]);
		P13A = _mm256_loadu_si256((__m256i*)&pred[13 * i_pred]);
		P14A = _mm256_loadu_si256((__m256i*)&pred[14 * i_pred]);
		P15A = _mm256_loadu_si256((__m256i*)&pred[15 * i_pred]);

		O00B = _mm256_loadu_si256((__m256i*)&org[0 * i_org + 16]);
		O01B = _mm256_loadu_si256((__m256i*)&org[1 * i_org + 16]);
		O02B = _mm256_loadu_si256((__m256i*)&org[2 * i_org + 16]);
		O03B = _mm256_loadu_si256((__m256i*)&org[3 * i_org + 16]);
		O04B = _mm256_loadu_si256((__m256i*)&org[4 * i_org + 16]);
		O05B = _mm256_loadu_si256((__m256i*)&org[5 * i_org + 16]);
		O06B = _mm256_loadu_si256((__m256i*)&org[6 * i_org + 16]);
		O07B = _mm256_loadu_si256((__m256i*)&org[7 * i_org + 16]);
		O08B = _mm256_loadu_si256((__m256i*)&org[8 * i_org + 16]);
		O09B = _mm256_loadu_si256((__m256i*)&org[9 * i_org + 16]);
		O10B = _mm256_loadu_si256((__m256i*)&org[10 * i_org + 16]);
		O11B = _mm256_loadu_si256((__m256i*)&org[11 * i_org + 16]);
		O12B = _mm256_loadu_si256((__m256i*)&org[12 * i_org + 16]);
		O13B = _mm256_loadu_si256((__m256i*)&org[13 * i_org + 16]);
		O14B = _mm256_loadu_si256((__m256i*)&org[14 * i_org + 16]);
		O15B = _mm256_loadu_si256((__m256i*)&org[15 * i_org + 16]);
		P00B = _mm256_loadu_si256((__m256i*)&pred[0 * i_pred + 16]);
		P01B = _mm256_loadu_si256((__m256i*)&pred[1 * i_pred + 16]);
		P02B = _mm256_loadu_si256((__m256i*)&pred[2 * i_pred + 16]);
		P03B = _mm256_loadu_si256((__m256i*)&pred[3 * i_pred + 16]);
		P04B = _mm256_loadu_si256((__m256i*)&pred[4 * i_pred + 16]);
		P05B = _mm256_loadu_si256((__m256i*)&pred[5 * i_pred + 16]);
		P06B = _mm256_loadu_si256((__m256i*)&pred[6 * i_pred + 16]);
		P07B = _mm256_loadu_si256((__m256i*)&pred[7 * i_pred + 16]);
		P08B = _mm256_loadu_si256((__m256i*)&pred[8 * i_pred + 16]);
		P09B = _mm256_loadu_si256((__m256i*)&pred[9 * i_pred + 16]);
		P10B = _mm256_loadu_si256((__m256i*)&pred[10 * i_pred + 16]);
		P11B = _mm256_loadu_si256((__m256i*)&pred[11 * i_pred + 16]);
		P12B = _mm256_loadu_si256((__m256i*)&pred[12 * i_pred + 16]);
		P13B = _mm256_loadu_si256((__m256i*)&pred[13 * i_pred + 16]);
		P14B = _mm256_loadu_si256((__m256i*)&pred[14 * i_pred + 16]);
		P15B = _mm256_loadu_si256((__m256i*)&pred[15 * i_pred + 16]);


		//SUB
		in[0][0] = _mm256_sub_epi16(O00A, P00A);
		in[0][1] = _mm256_sub_epi16(O00B, P00B);
		in[1][0] = _mm256_sub_epi16(O01A, P01A);
		in[1][1] = _mm256_sub_epi16(O01B, P01B);
		in[2][0] = _mm256_sub_epi16(O02A, P02A);
		in[2][1] = _mm256_sub_epi16(O02B, P02B);
		in[3][0] = _mm256_sub_epi16(O03A, P03A);
		in[3][1] = _mm256_sub_epi16(O03B, P03B);
		in[4][0] = _mm256_sub_epi16(O04A, P04A);
		in[4][1] = _mm256_sub_epi16(O04B, P04B);
		in[5][0] = _mm256_sub_epi16(O05A, P05A);
		in[5][1] = _mm256_sub_epi16(O05B, P05B);
		in[6][0] = _mm256_sub_epi16(O06A, P06A);
		in[6][1] = _mm256_sub_epi16(O06B, P06B);
		in[7][0] = _mm256_sub_epi16(O07A, P07A);
		in[7][1] = _mm256_sub_epi16(O07B, P07B);

		in[8][0] = _mm256_sub_epi16(O08A, P08A);
		in[8][1] = _mm256_sub_epi16(O08B, P08B);
		in[9][0] = _mm256_sub_epi16(O09A, P09A);
		in[9][1] = _mm256_sub_epi16(O09B, P09B);
		in[10][0] = _mm256_sub_epi16(O10A, P10A);
		in[10][1] = _mm256_sub_epi16(O10B, P10B);
		in[11][0] = _mm256_sub_epi16(O11A, P11A);
		in[11][1] = _mm256_sub_epi16(O11B, P11B);
		in[12][0] = _mm256_sub_epi16(O12A, P12A);
		in[12][1] = _mm256_sub_epi16(O12B, P12B);
		in[13][0] = _mm256_sub_epi16(O13A, P13A);
		in[13][1] = _mm256_sub_epi16(O13B, P13B);
		in[14][0] = _mm256_sub_epi16(O14A, P14A);
		in[14][1] = _mm256_sub_epi16(O14B, P14B);
		in[15][0] = _mm256_sub_epi16(O15A, P15A);
		in[15][1] = _mm256_sub_epi16(O15B, P15B);

		T00A = _mm256_permute2x128_si256(in[0][0], in[8][0], 0x20);
		T01A = _mm256_permute2x128_si256(in[1][0], in[9][0], 0x20);
		T02A = _mm256_permute2x128_si256(in[2][0], in[10][0], 0x20);
		T03A = _mm256_permute2x128_si256(in[3][0], in[11][0], 0x20);
		T04A = _mm256_permute2x128_si256(in[4][0], in[12][0], 0x20);
		T05A = _mm256_permute2x128_si256(in[5][0], in[13][0], 0x20);
		T06A = _mm256_permute2x128_si256(in[6][0], in[14][0], 0x20);
		T07A = _mm256_permute2x128_si256(in[7][0], in[15][0], 0x20);

		T00B = _mm256_permute2x128_si256(in[0][0], in[8][0], 0x31);
		T01B = _mm256_permute2x128_si256(in[1][0], in[9][0], 0x31);
		T02B = _mm256_permute2x128_si256(in[2][0], in[10][0], 0x31);
		T03B = _mm256_permute2x128_si256(in[3][0], in[11][0], 0x31);
		T04B = _mm256_permute2x128_si256(in[4][0], in[12][0], 0x31);
		T05B = _mm256_permute2x128_si256(in[5][0], in[13][0], 0x31);
		T06B = _mm256_permute2x128_si256(in[6][0], in[14][0], 0x31);
		T07B = _mm256_permute2x128_si256(in[7][0], in[15][0], 0x31);

		T00C = _mm256_permute2x128_si256(in[0][1], in[8][1], 0x20);
		T01C = _mm256_permute2x128_si256(in[1][1], in[9][1], 0x20);
		T02C = _mm256_permute2x128_si256(in[2][1], in[10][1], 0x20);
		T03C = _mm256_permute2x128_si256(in[3][1], in[11][1], 0x20);
		T04C = _mm256_permute2x128_si256(in[4][1], in[12][1], 0x20);
		T05C = _mm256_permute2x128_si256(in[5][1], in[13][1], 0x20);
		T06C = _mm256_permute2x128_si256(in[6][1], in[14][1], 0x20);
		T07C = _mm256_permute2x128_si256(in[7][1], in[15][1], 0x20);

		T00D = _mm256_permute2x128_si256(in[0][1], in[8][1], 0x31);
		T01D = _mm256_permute2x128_si256(in[1][1], in[9][1], 0x31);
		T02D = _mm256_permute2x128_si256(in[2][1], in[10][1], 0x31);
		T03D = _mm256_permute2x128_si256(in[3][1], in[11][1], 0x31);
		T04D = _mm256_permute2x128_si256(in[4][1], in[12][1], 0x31);
		T05D = _mm256_permute2x128_si256(in[5][1], in[13][1], 0x31);
		T06D = _mm256_permute2x128_si256(in[6][1], in[14][1], 0x31);
		T07D = _mm256_permute2x128_si256(in[7][1], in[15][1], 0x31);
...
#undef MAKE_ODD
	...
	//for (i = 0; i < 32; i++){
	//	_mm256_storeu_si256((__m256i*)&dst[i * 32], im[i][0]);
	//	_mm256_storeu_si256((__m256i*)&dst[i * 32 + 16], im[i][1]);
	//}
...
void sub_trans_64x64_10bit_sse128(pel_t *org, int i_org, pel_t *pred, int i_pred, coef_t *dst, int shift){
	...
	__m128i tt;
    __m128i c_8 = _mm_set1_epi32(1 << (shift - 1));
	__m128i c_512 = _mm_set1_epi32(512);
	int shift1 = shift;
	int shift2 = 10;
	...
	for (i = 0; i < 8; i++){
        ...
        pel_t *pred16 = pred + 16;
        pel_t *pred32 = pred + 32;
        pel_t *pred48 = pred + 48;
        ...
        O00A = _mm_loadu_si128((__m128i*)&org16[0 * i_org]);
        O00B = _mm_loadu_si128((__m128i*)&org16[0 * i_org + 8]);
        O01A = _mm_loadu_si128((__m128i*)&org16[1 * i_org]);
        O01B = _mm_loadu_si128((__m128i*)&org16[1 * i_org + 8]);
        O02A = _mm_loadu_si128((__m128i*)&org16[2 * i_org]);
        O02B = _mm_loadu_si128((__m128i*)&org16[2 * i_org + 8]);
        O03A = _mm_loadu_si128((__m128i*)&org16[3 * i_org]);
        O03B = _mm_loadu_si128((__m128i*)&org16[3 * i_org + 8]);
        O04A = _mm_loadu_si128((__m128i*)&org16[4 * i_org]);
        O04B = _mm_loadu_si128((__m128i*)&org16[4 * i_org + 8]);
        O05A = _mm_loadu_si128((__m128i*)&org16[5 * i_org]);
        O05B = _mm_loadu_si128((__m128i*)&org16[5 * i_org + 8]);
        O06A = _mm_loadu_si128((__m128i*)&org16[6 * i_org]);
        O06B = _mm_loadu_si128((__m128i*)&org16[6 * i_org + 8]);
        O07A = _mm_loadu_si128((__m128i*)&org16[7 * i_org]);
        O07B = _mm_loadu_si128((__m128i*)&org16[7 * i_org + 8]);

        P00A = _mm_loadu_si128((__m128i*)&pred16[0 * i_pred]);
        P00B = _mm_loadu_si128((__m128i*)&pred16[0 * i_pred + 8]);
        P01A = _mm_loadu_si128((__m128i*)&pred16[1 * i_pred]);
        P01B = _mm_loadu_si128((__m128i*)&pred16[1 * i_pred + 8]);
        P02A = _mm_loadu_si128((__m128i*)&pred16[2 * i_pred]);
        P02B = _mm_loadu_si128((__m128i*)&pred16[2 * i_pred + 8]);
        P03A = _mm_loadu_si128((__m128i*)&pred16[3 * i_pred]);
        P03B = _mm_loadu_si128((__m128i*)&pred16[3 * i_pred + 8]);
        P04A = _mm_loadu_si128((__m128i*)&pred16[4 * i_pred]);
        P04B = _mm_loadu_si128((__m128i*)&pred16[4 * i_pred + 8]);
        P05A = _mm_loadu_si128((__m128i*)&pred16[5 * i_pred]);
        P05B = _mm_loadu_si128((__m128i*)&pred16[5 * i_pred + 8]);
        P06A = _mm_loadu_si128((__m128i*)&pred16[6 * i_pred]);
        P06B = _mm_loadu_si128((__m128i*)&pred16[6 * i_pred + 8]);
        P07A = _mm_loadu_si128((__m128i*)&pred16[7 * i_pred]);
        P07B = _mm_loadu_si128((__m128i*)&pred16[7 * i_pred + 8]);
        ...
        O00A = _mm_loadu_si128((__m128i*)&org32[0 * i_org]);
        O00B = _mm_loadu_si128((__m128i*)&org32[0 * i_org + 8]);
        O01A = _mm_loadu_si128((__m128i*)&org32[1 * i_org]);
        O01B = _mm_loadu_si128((__m128i*)&org32[1 * i_org + 8]);
        O02A = _mm_loadu_si128((__m128i*)&org32[2 * i_org]);
        O02B = _mm_loadu_si128((__m128i*)&org32[2 * i_org + 8]);
        O03A = _mm_loadu_si128((__m128i*)&org32[3 * i_org]);
        O03B = _mm_loadu_si128((__m128i*)&org32[3 * i_org + 8]);
        O04A = _mm_loadu_si128((__m128i*)&org32[4 * i_org]);
        O04B = _mm_loadu_si128((__m128i*)&org32[4 * i_org + 8]);
        O05A = _mm_loadu_si128((__m128i*)&org32[5 * i_org]);
        O05B = _mm_loadu_si128((__m128i*)&org32[5 * i_org + 8]);
        O06A = _mm_loadu_si128((__m128i*)&org32[6 * i_org]);
        O06B = _mm_loadu_si128((__m128i*)&org32[6 * i_org + 8]);
        O07A = _mm_loadu_si128((__m128i*)&org32[7 * i_org]);
        O07B = _mm_loadu_si128((__m128i*)&org32[7 * i_org + 8]);

        P00A = _mm_loadu_si128((__m128i*)&pred32[0 * i_pred]);
        P00B = _mm_loadu_si128((__m128i*)&pred32[0 * i_pred + 8]);
        P01A = _mm_loadu_si128((__m128i*)&pred32[1 * i_pred]);
        P01B = _mm_loadu_si128((__m128i*)&pred32[1 * i_pred + 8]);
        P02A = _mm_loadu_si128((__m128i*)&pred32[2 * i_pred]);
        P02B = _mm_loadu_si128((__m128i*)&pred32[2 * i_pred + 8]);
        P03A = _mm_loadu_si128((__m128i*)&pred32[3 * i_pred]);
        P03B = _mm_loadu_si128((__m128i*)&pred32[3 * i_pred + 8]);
        P04A = _mm_loadu_si128((__m128i*)&pred32[4 * i_pred]);
        P04B = _mm_loadu_si128((__m128i*)&pred32[4 * i_pred + 8]);
        P05A = _mm_loadu_si128((__m128i*)&pred32[5 * i_pred]);
        P05B = _mm_loadu_si128((__m128i*)&pred32[5 * i_pred + 8]);
        P06A = _mm_loadu_si128((__m128i*)&pred32[6 * i_pred]);
        P06B = _mm_loadu_si128((__m128i*)&pred32[6 * i_pred + 8]);
        P07A = _mm_loadu_si128((__m128i*)&pred32[7 * i_pred]);
        P07B = _mm_loadu_si128((__m128i*)&pred32[7 * i_pred + 8]);
        ...
        O00A = _mm_loadu_si128((__m128i*)&org48[0 * i_org]);
        O00B = _mm_loadu_si128((__m128i*)&org48[0 * i_org + 8]);
        O01A = _mm_loadu_si128((__m128i*)&org48[1 * i_org]);
        O01B = _mm_loadu_si128((__m128i*)&org48[1 * i_org + 8]);
        O02A = _mm_loadu_si128((__m128i*)&org48[2 * i_org]);
        O02B = _mm_loadu_si128((__m128i*)&org48[2 * i_org + 8]);
        O03A = _mm_loadu_si128((__m128i*)&org48[3 * i_org]);
        O03B = _mm_loadu_si128((__m128i*)&org48[3 * i_org + 8]);
        O04A = _mm_loadu_si128((__m128i*)&org48[4 * i_org]);
        O04B = _mm_loadu_si128((__m128i*)&org48[4 * i_org + 8]);
        O05A = _mm_loadu_si128((__m128i*)&org48[5 * i_org]);
        O05B = _mm_loadu_si128((__m128i*)&org48[5 * i_org + 8]);
        O06A = _mm_loadu_si128((__m128i*)&org48[6 * i_org]);
        O06B = _mm_loadu_si128((__m128i*)&org48[6 * i_org + 8]);
        O07A = _mm_loadu_si128((__m128i*)&org48[7 * i_org]);
        O07B = _mm_loadu_si128((__m128i*)&org48[7 * i_org + 8]);

        P00A = _mm_loadu_si128((__m128i*)&pred48[0 * i_pred]);
        P00B = _mm_loadu_si128((__m128i*)&pred48[0 * i_pred + 8]);
        P01A = _mm_loadu_si128((__m128i*)&pred48[1 * i_pred]);
        P01B = _mm_loadu_si128((__m128i*)&pred48[1 * i_pred + 8]);
        P02A = _mm_loadu_si128((__m128i*)&pred48[2 * i_pred]);
        P02B = _mm_loadu_si128((__m128i*)&pred48[2 * i_pred + 8]);
        P03A = _mm_loadu_si128((__m128i*)&pred48[3 * i_pred]);
        P03B = _mm_loadu_si128((__m128i*)&pred48[3 * i_pred + 8]);
        P04A = _mm_loadu_si128((__m128i*)&pred48[4 * i_pred]);
        P04B = _mm_loadu_si128((__m128i*)&pred48[4 * i_pred + 8]);
        P05A = _mm_loadu_si128((__m128i*)&pred48[5 * i_pred]);
        P05B = _mm_loadu_si128((__m128i*)&pred48[5 * i_pred + 8]);
        P06A = _mm_loadu_si128((__m128i*)&pred48[6 * i_pred]);
        P06B = _mm_loadu_si128((__m128i*)&pred48[6 * i_pred + 8]);
        P07A = _mm_loadu_si128((__m128i*)&pred48[7 * i_pred]);
        P07B = _mm_loadu_si128((__m128i*)&pred48[7 * i_pred + 8]);
...
void sub_trans_64x64_10bit_sse256(pel_t *org, int i_org, pel_t *pred, int i_pred, coef_t *dst, int shift){
	...
	int shift1 = shift;
	int shift2 = 10;
	__m256i c_8 = _mm256_set1_epi32(1 << (shift - 1));
	__m256i c_512 = _mm256_set1_epi32(512);

	//// pix sub & wavelet ////
	for (i = 0; i < 4; i++){
		//load data
		O00 = _mm256_loadu_si256((__m256i*)&org[0 * i_org]);
		...
		T[i * 16 + 0][0] = _mm256_sub_epi16(O00, P00);
		T[i * 16 + 1][0] = _mm256_sub_epi16(O01, P01);
		T[i * 16 + 2][0] = _mm256_sub_epi16(O02, P02);
		T[i * 16 + 3][0] = _mm256_sub_epi16(O03, P03);
		T[i * 16 + 4][0] = _mm256_sub_epi16(O04, P04);
		T[i * 16 + 5][0] = _mm256_sub_epi16(O05, P05);
		T[i * 16 + 6][0] = _mm256_sub_epi16(O06, P06);
		T[i * 16 + 7][0] = _mm256_sub_epi16(O07, P07);

		O00 = _mm256_loadu_si256((__m256i*)&org[0 * i_org + 16]);
		O01 = _mm256_loadu_si256((__m256i*)&org[1 * i_org + 16]);
		O02 = _mm256_loadu_si256((__m256i*)&org[2 * i_org + 16]);
		O03 = _mm256_loadu_si256((__m256i*)&org[3 * i_org + 16]);
		O04 = _mm256_loadu_si256((__m256i*)&org[4 * i_org + 16]);
		O05 = _mm256_loadu_si256((__m256i*)&org[5 * i_org + 16]);
		O06 = _mm256_loadu_si256((__m256i*)&org[6 * i_org + 16]);
		O07 = _mm256_loadu_si256((__m256i*)&org[7 * i_org + 16]);
		P00 = _mm256_loadu_si256((__m256i*)&pred[0 * i_pred + 16]);
		P01 = _mm256_loadu_si256((__m256i*)&pred[1 * i_pred + 16]);
		P02 = _mm256_loadu_si256((__m256i*)&pred[2 * i_pred + 16]);
		P03 = _mm256_loadu_si256((__m256i*)&pred[3 * i_pred + 16]);
		P04 = _mm256_loadu_si256((__m256i*)&pred[4 * i_pred + 16]);
		P05 = _mm256_loadu_si256((__m256i*)&pred[5 * i_pred + 16]);
		P06 = _mm256_loadu_si256((__m256i*)&pred[6 * i_pred + 16]);
		P07 = _mm256_loadu_si256((__m256i*)&pred[7 * i_pred + 16]);

		T[i * 16 + 0][1] = _mm256_sub_epi16(O00, P00);
		T[i * 16 + 1][1] = _mm256_sub_epi16(O01, P01);
		T[i * 16 + 2][1] = _mm256_sub_epi16(O02, P02);
		T[i * 16 + 3][1] = _mm256_sub_epi16(O03, P03);
		T[i * 16 + 4][1] = _mm256_sub_epi16(O04, P04);
		T[i * 16 + 5][1] = _mm256_sub_epi16(O05, P05);
		T[i * 16 + 6][1] = _mm256_sub_epi16(O06, P06);
		T[i * 16 + 7][1] = _mm256_sub_epi16(O07, P07);
		...
		T[i * 16 + 0][2] = _mm256_sub_epi16(O00, P00);
		T[i * 16 + 1][2] = _mm256_sub_epi16(O01, P01);
		T[i * 16 + 2][2] = _mm256_sub_epi16(O02, P02);
		T[i * 16 + 3][2] = _mm256_sub_epi16(O03, P03);
		T[i * 16 + 4][2] = _mm256_sub_epi16(O04, P04);
		T[i * 16 + 5][2] = _mm256_sub_epi16(O05, P05);
		T[i * 16 + 6][2] = _mm256_sub_epi16(O06, P06);
		T[i * 16 + 7][2] = _mm256_sub_epi16(O07, P07);

		O00 = _mm256_loadu_si256((__m256i*)&org[0 * i_org + 48]);
		O01 = _mm256_loadu_si256((__m256i*)&org[1 * i_org + 48]);
		O02 = _mm256_loadu_si256((__m256i*)&org[2 * i_org + 48]);
		O03 = _mm256_loadu_si256((__m256i*)&org[3 * i_org + 48]);
		O04 = _mm256_loadu_si256((__m256i*)&org[4 * i_org + 48]);
		O05 = _mm256_loadu_si256((__m256i*)&org[5 * i_org + 48]);
		O06 = _mm256_loadu_si256((__m256i*)&org[6 * i_org + 48]);
		O07 = _mm256_loadu_si256((__m256i*)&org[7 * i_org + 48]);
		P00 = _mm256_loadu_si256((__m256i*)&pred[0 * i_pred + 48]);
		P01 = _mm256_loadu_si256((__m256i*)&pred[1 * i_pred + 48]);
		P02 = _mm256_loadu_si256((__m256i*)&pred[2 * i_pred + 48]);
		P03 = _mm256_loadu_si256((__m256i*)&pred[3 * i_pred + 48]);
		P04 = _mm256_loadu_si256((__m256i*)&pred[4 * i_pred + 48]);
		P05 = _mm256_loadu_si256((__m256i*)&pred[5 * i_pred + 48]);
		P06 = _mm256_loadu_si256((__m256i*)&pred[6 * i_pred + 48]);
		P07 = _mm256_loadu_si256((__m256i*)&pred[7 * i_pred + 48]);

		T[i * 16 + 0][3] = _mm256_sub_epi16(O00, P00);
		T[i * 16 + 1][3] = _mm256_sub_epi16(O01, P01);
		T[i * 16 + 2][3] = _mm256_sub_epi16(O02, P02);
		T[i * 16 + 3][3] = _mm256_sub_epi16(O03, P03);
		T[i * 16 + 4][3] = _mm256_sub_epi16(O04, P04);
		T[i * 16 + 5][3] = _mm256_sub_epi16(O05, P05);
		T[i * 16 + 6][3] = _mm256_sub_epi16(O06, P06);
		T[i * 16 + 7][3] = _mm256_sub_epi16(O07, P07);

		O00 = _mm256_loadu_si256((__m256i*)&org[8 * i_org]);
		...
		T[i * 16 + 8][0] = _mm256_sub_epi16(O00, P00);
		T[i * 16 + 9][0] = _mm256_sub_epi16(O01, P01);
		T[i * 16 + 10][0] = _mm256_sub_epi16(O02, P02);
		T[i * 16 + 11][0] = _mm256_sub_epi16(O03, P03);
		T[i * 16 + 12][0] = _mm256_sub_epi16(O04, P04);
		T[i * 16 + 13][0] = _mm256_sub_epi16(O05, P05);
		T[i * 16 + 14][0] = _mm256_sub_epi16(O06, P06);
		T[i * 16 + 15][0] = _mm256_sub_epi16(O07, P07);

		O00 = _mm256_loadu_si256((__m256i*)&org[8 * i_org + 16]);
		O01 = _mm256_loadu_si256((__m256i*)&org[9 * i_org + 16]);
		O02 = _mm256_loadu_si256((__m256i*)&org[10 * i_org + 16]);
		O03 = _mm256_loadu_si256((__m256i*)&org[11 * i_org + 16]);
		O04 = _mm256_loadu_si256((__m256i*)&org[12 * i_org + 16]);
		O05 = _mm256_loadu_si256((__m256i*)&org[13 * i_org + 16]);
		O06 = _mm256_loadu_si256((__m256i*)&org[14 * i_org + 16]);
		O07 = _mm256_loadu_si256((__m256i*)&org[15 * i_org + 16]);
		P00 = _mm256_loadu_si256((__m256i*)&pred[8 * i_pred + 16]);
		P01 = _mm256_loadu_si256((__m256i*)&pred[9 * i_pred + 16]);
		P02 = _mm256_loadu_si256((__m256i*)&pred[10 * i_pred + 16]);
		P03 = _mm256_loadu_si256((__m256i*)&pred[11 * i_pred + 16]);
		P04 = _mm256_loadu_si256((__m256i*)&pred[12 * i_pred + 16]);
		P05 = _mm256_loadu_si256((__m256i*)&pred[13 * i_pred + 16]);
		P06 = _mm256_loadu_si256((__m256i*)&pred[14 * i_pred + 16]);
		P07 = _mm256_loadu_si256((__m256i*)&pred[15 * i_pred + 16]);

		T[i * 16 + 8][1] = _mm256_sub_epi16(O00, P00);
		T[i * 16 + 9][1] = _mm256_sub_epi16(O01, P01);
		T[i * 16 + 10][1] = _mm256_sub_epi16(O02, P02);
		T[i * 16 + 11][1] = _mm256_sub_epi16(O03, P03);
		T[i * 16 + 12][1] = _mm256_sub_epi16(O04, P04);
		T[i * 16 + 13][1] = _mm256_sub_epi16(O05, P05);
		T[i * 16 + 14][1] = _mm256_sub_epi16(O06, P06);
		T[i * 16 + 15][1] = _mm256_sub_epi16(O07, P07);
		...
		T[i * 16 + 8][2] = _mm256_sub_epi16(O00, P00);
		T[i * 16 + 9][2] = _mm256_sub_epi16(O01, P01);
		T[i * 16 + 10][2] = _mm256_sub_epi16(O02, P02);
		T[i * 16 + 11][2] = _mm256_sub_epi16(O03, P03);
		T[i * 16 + 12][2] = _mm256_sub_epi16(O04, P04);
		T[i * 16 + 13][2] = _mm256_sub_epi16(O05, P05);
		T[i * 16 + 14][2] = _mm256_sub_epi16(O06, P06);
		T[i * 16 + 15][2] = _mm256_sub_epi16(O07, P07);

		O00 = _mm256_loadu_si256((__m256i*)&org[8 * i_org + 48]);
		O01 = _mm256_loadu_si256((__m256i*)&org[9 * i_org + 48]);
		O02 = _mm256_loadu_si256((__m256i*)&org[10 * i_org + 48]);
		O03 = _mm256_loadu_si256((__m256i*)&org[11 * i_org + 48]);
		O04 = _mm256_loadu_si256((__m256i*)&org[12 * i_org + 48]);
		O05 = _mm256_loadu_si256((__m256i*)&org[13 * i_org + 48]);
		O06 = _mm256_loadu_si256((__m256i*)&org[14 * i_org + 48]);
		O07 = _mm256_loadu_si256((__m256i*)&org[15 * i_org + 48]);
		P00 = _mm256_loadu_si256((__m256i*)&pred[8 * i_pred + 48]);
		P01 = _mm256_loadu_si256((__m256i*)&pred[9 * i_pred + 48]);
		P02 = _mm256_loadu_si256((__m256i*)&pred[10 * i_pred + 48]);
		P03 = _mm256_loadu_si256((__m256i*)&pred[11 * i_pred + 48]);
		P04 = _mm256_loadu_si256((__m256i*)&pred[12 * i_pred + 48]);
		P05 = _mm256_loadu_si256((__m256i*)&pred[13 * i_pred + 48]);
		P06 = _mm256_loadu_si256((__m256i*)&pred[14 * i_pred + 48]);
		P07 = _mm256_loadu_si256((__m256i*)&pred[15 * i_pred + 48]);

		T[i * 16 + 8][3] = _mm256_sub_epi16(O00, P00);
		T[i * 16 + 9][3] = _mm256_sub_epi16(O01, P01);
		T[i * 16 + 10][3] = _mm256_sub_epi16(O02, P02);
		T[i * 16 + 11][3] = _mm256_sub_epi16(O03, P03);
		T[i * 16 + 12][3] = _mm256_sub_epi16(O04, P04);
		T[i * 16 + 13][3] = _mm256_sub_epi16(O05, P05);
		T[i * 16 + 14][3] = _mm256_sub_epi16(O06, P06);
		T[i * 16 + 15][3] = _mm256_sub_epi16(O07, P07);

		org += i_org << 4;
		pred += i_pred << 4;

	//	for (int j = 0; j < 16; j++){
	//		_mm256_storeu_si256((__m256*)&dst[(i * 16 + j) * 64 + 0], T[i * 16 + j][0]);
	//		_mm256_storeu_si256((__m256*)&dst[(i * 16 + j) * 64 + 16], T[i * 16 + j][1]);
	//		_mm256_storeu_si256((__m256*)&dst[(i * 16 + j) * 64 + 32], T[i * 16 + j][2]);
	//		_mm256_storeu_si256((__m256*)&dst[(i * 16 + j) * 64 + 48], T[i * 16 + j][3]);
	//	}
	//}
...
#define MAKE_ODD(tab0, tab1, tab2, tab3, dstPos) \
    ...
    \
    tResult = _mm_packs_epi32(T60, TT60); \
    _mm_storeu_si128((__m128i*)&dst[((dstPos) << 5) + idx], tResult); \
...
void sub_trans_ext_64x64_10bit_sse256(pel_t *org, int i_org, pel_t *pred, int i_pred, coef_t *dst, int shift){
	...
	int shift1 = shift;
	int shift2 = 10;
	__m256i c_8 = _mm256_set1_epi32(1 << (shift1 - 1));
	__m256i c_512 = _mm256_set1_epi32(512);
