        ...
        for (j = 0; j < height; j += 2) {
            ...
            for (k = 0; k < width; k += 8) {
                ...
                if (j < 16 && k < 16) {
                    ...
                    mm_c[1] = _mm256_mullo_epi16(mm_idx_k_s16, mm_c[0]);
                    mm_c[3] = _mm256_mullo_epi16(mm_idx_k_s16, mm_c[2]);
                    mm_c[4] = _mm256_mullo_epi16(mm_idx_j_s16, mm_c[0]);
                    mm_c[5] = _mm256_mullo_epi16(mm_idx_j_s16, mm_c[2]);
                    mm_c[6] = _mm256_set_m128i(m5, m4);

                    mm_tmp[0] = _mm256_madd_epi16(mm_c[0], mm_c[0]);    // (11 + 11 + 1)bits
                    mm_tmp[1] = _mm256_madd_epi16(mm_c[0], mm_c[1]);    // (11 + 15 + 1)bits
                    mm_tmp[2] = _mm256_madd_epi16(mm_c[0], mm_c[2]);    // (11 + 11 + 1)bits
                    mm_tmp[3] = _mm256_madd_epi16(mm_c[0], mm_c[3]);    // (11 + 15 + 1)bits
                    mm_tmp[4] = _mm256_madd_epi16(mm_c[0], mm_c[4]);    // (11 + 15 + 1)bits
                    mm_tmp[5] = _mm256_madd_epi16(mm_c[0], mm_c[5]);    // (11 + 15 + 1)bits
                    mm_tmp[6] = _mm256_madd_epi16(mm_c[0], mm_c[6]);    // (11 + 8 + 1)bits
                    mm_tmp[0] = _mm256_hadd_epi32(mm_tmp[0], mm_tmp[1]);
                    mm_tmp[2] = _mm256_hadd_epi32(mm_tmp[2], mm_tmp[3]);
                    mm_tmp[4] = _mm256_hadd_epi32(mm_tmp[4], mm_tmp[5]);
                    mm_tmp[6] = _mm256_hadd_epi32(mm_tmp[6], mm_tmp[6]);
                    mm_tmp[0] = _mm256_hadd_epi32(mm_tmp[0], mm_tmp[2]);
                    mm_tmp[4] = _mm256_hadd_epi32(mm_tmp[4], mm_tmp[6]);
                    m6 = _mm_add_epi32(_mm256_castsi256_si128(mm_tmp[0]), _mm256_extracti128_si256(mm_tmp[0], 1));
                    m7 = _mm_add_epi32(_mm256_castsi256_si128(mm_tmp[4]), _mm256_extracti128_si256(mm_tmp[4], 1));
                    mm_tmp[0] = _mm256_loadu_si256((const __m256i*)&coef[1][0]);
                    mm_tmp[1] = _mm256_cvtepi32_epi64(m6);
                    mm_tmp[0] = _mm256_add_epi64(mm_tmp[0], mm_tmp[1]);
                    _mm256_storeu_si256((__m256i*)&coef[1][0], mm_tmp[0]);
                    coef[1][4] += _mm_extract_epi32(m7, 0);
                    coef[1][5] += _mm_extract_epi32(m7, 1);
                    coef[1][6] += _mm_extract_epi32(m7, 2);

                    mm_tmp[0] = _mm256_madd_epi16(mm_c[2], mm_c[0]);    // (11 + 11 + 1)bits
                    mm_tmp[1] = _mm256_madd_epi16(mm_c[2], mm_c[1]);    // (11 + 15 + 1)bits
                    mm_tmp[2] = _mm256_madd_epi16(mm_c[2], mm_c[2]);    // (11 + 11 + 1)bits
                    mm_tmp[3] = _mm256_madd_epi16(mm_c[2], mm_c[3]);    // (11 + 15 + 1)bits
                    mm_tmp[4] = _mm256_madd_epi16(mm_c[2], mm_c[4]);    // (11 + 15 + 1)bits
                    mm_tmp[5] = _mm256_madd_epi16(mm_c[2], mm_c[5]);    // (11 + 15 + 1)bits
                    mm_tmp[6] = _mm256_madd_epi16(mm_c[2], mm_c[6]);    // (11 + 8 + 1)bits
                    mm_tmp[0] = _mm256_hadd_epi32(mm_tmp[0], mm_tmp[1]);
                    mm_tmp[2] = _mm256_hadd_epi32(mm_tmp[2], mm_tmp[3]);
                    mm_tmp[4] = _mm256_hadd_epi32(mm_tmp[4], mm_tmp[5]);
                    mm_tmp[6] = _mm256_hadd_epi32(mm_tmp[6], mm_tmp[6]);
                    ...
                    mm_tmp[1] = _mm256_cvtepi32_epi64(m6);
                    mm_tmp[0] = _mm256_add_epi64(mm_tmp[0], mm_tmp[1]);
                    _mm256_storeu_si256((__m256i*)&coef[3][0], mm_tmp[0]);
                    coef[3][4] += _mm_extract_epi32(m7, 0);
                    coef[3][5] += _mm_extract_epi32(m7, 1);
                    coef[3][6] += _mm_extract_epi32(m7, 2);
